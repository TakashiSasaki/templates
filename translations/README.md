...
English is the canonical language of maintained Webapp documentation. Files under `translations/<language>/` are non-authoritative translations and must not define independent requirements, navigation structure, or copyable-template content.
...
Translations of files beneath `template/` live beneath `translations/<language>/template/`; they are documentation-only derivatives and are never copied into the Webapp `template/` distribution.
