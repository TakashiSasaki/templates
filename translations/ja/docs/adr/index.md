...
* [メンテナー公開ソース](../publication-catalog.md#deferred-maintainer-publications) - ADR-0008 は信頼と来歴について現行であり、レビュー結果の表現については ADR-0009 により一部が置き換えられています。両決定は現在 Policy の公開カタログに含まれており、統合された読者向け公開は Site の staging と後続 promotion によって制御されます。
...
* [ADR-0008: レビュー権威と GitHub ランタイムの境界（一部置き換え済み）](0008-review-authority-and-github-runtime-boundary.md) - 信頼されたレビューのブートストラップ、来歴、GitHub ランタイム境界を保持します。置き換えられた表現要件は ADR-0009 を参照してください。
* [ADR-0009: レビュー結果表現の境界](0009-review-result-representation-boundary.md) - 識別子に結び付いた完了ハンドオフを保持しつつ、表現をレビュー権威の外に置きます。ADR-0008 を部分的に置き換えます。
