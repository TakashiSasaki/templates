...
class TranslationError(RuntimeError):
    """Raised when translation metadata or content violates the contract."""
...
def validate_japanese_notice(path: Path, translation: PurePosixPath) -> None:
    ...
    if index >= len(lines) or not lines[index].startswith(JA_NOTICE):
        raise TranslationError(
            "Japanese translation must place the non-authoritative notice immediately "
            f"after its top-level title: {translation}"
        )
...
def validate(root: Path) -> list[str]:
    ...
    for index, entry in enumerate(entries):
        ...
        if translation != expected:
            raise TranslationError(
                f"{field}.translation must mirror the canonical path at {expected}"
            )
