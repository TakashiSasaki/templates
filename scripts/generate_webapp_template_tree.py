#!/usr/bin/env python3
"""Generate the Webapp copyable-template subtree page from a locked checkout."""
...
TREE_MARKER = "<!-- GENERATED_WEBAPP_TEMPLATE_TREE -->"
SUMMARY_MARKER = "<!-- GENERATED_WEBAPP_TEMPLATE_SUMMARY -->"
...
def generate(
    ...
    root = webapp_root.resolve(strict=True)
    ...
    if distribution is None or not distribution.is_directory:
        raise RepositoryTreeError(
            "webapp repository does not contain a tracked template directory"
        )

    docs_root = output_root / "docs" / "repository-trees"
    page = docs_root / "webapp" / "template.md"
    index = docs_root / "index.md"
    if not page.is_file() or not index.is_file():
        raise RepositoryTreeError(
            "assembled Webapp template-tree templates are missing"
        )

    site_base_path = configured_base_path(output_root / "zensical.toml")
    published = published_sources("webapp", root, site_root)
    ...
    summary = (
        "| [Web application copyable template](webapp/template.md) | "
    ...
    return f"webapp template: {counts['files']} files at {revision}"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--webapp-root", type=Path, required=True)
...
def main() -> int:
    ...
    try:
        message = generate(
            ...
            arguments.webapp_root,
        )
    except (OSError, RepositoryTreeError) as exc:
        raise SystemExit(f"webapp template-tree generation failed: {exc}") from exc
    print(message)
    return 0
