...
def deployment_timestamp_from_index(site_root: Path) -> str:
    ...
    timestamps = [match.group("timestamp") for match in DEPLOYMENT_NOTICE_PATTERN.finditer(visible_text)]
...
def parse_publications(values: list[str]) -> dict[str, str]:
    ...
    for value in values:
        ...
        if name not in EXPECTED_PUBLICATIONS:
            raise FreshnessMetadataError(f"unsupported publication: {name!r}")
        if name in publications:
            raise FreshnessMetadataError(f"duplicate publication: {name!r}")
        publications[name] = validate_revision(revision, name)
    ...
    return {name: publications[name] for name in EXPECTED_PUBLICATIONS}
...
def annotate_generated_html(site_root: Path, site_revision: str) -> int:
    ...
    for path in html_files:
        ...
        updates[path] = annotate_site_revision(source, site_revision, path)
...
def build_payload(
    ...
    normalized_publications = parse_publications(
        [f"{name}={revision}" for name, revision in publications.items()]
    )
    return {
        "schema_version": 1,
