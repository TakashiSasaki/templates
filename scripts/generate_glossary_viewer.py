...
PROVIDER_LABELS = {"site": "Site", "skill": "Skill", "policy": "Policy", "webapp": "Webapp"}
ROOT_KEYS = {"schema_version", "repository", "terms"}
TERM_KEYS = {"id", "term", "aliases", "localized_labels", "origin", "definition", "summary", "authority", "repository_usage", "related_terms", "provider", "source_path", "source_revision"}
...
def _validate_repository(value: Any) -> str:
    ...
    for component in (owner, name):
        if component in {".", ".."} or GITHUB_REPOSITORY_COMPONENT.fullmatch(component) is None:
            raise GlossaryViewerError("repository must use safe owner/name path segments")
    return repository
...
def _validate_https_url(value: Any, field: str) -> str:
    ...
    if parsed.scheme != "https" or not parsed.netloc or not parsed.hostname or parsed.username is not None or parsed.password is not None:
        raise GlossaryViewerError(f"{field} must be a valid HTTPS URL")
    return url
...
def _parse_localized_labels(value: Any, field: str) -> dict[str, dict[str, Any]]:
    ...
    for language, raw in value.items():
        if not isinstance(language, str) or LANGUAGE_TAG.fullmatch(language) is None:
            raise GlossaryViewerError(f"{field} contains an invalid language tag: {language}")
        ...
        if normalized_language in normalized_languages:
            raise GlossaryViewerError(f"{field} contains duplicate language tags ignoring case: {language}")
        normalized_languages.add(normalized_language)
        if normalized_language == "en" or normalized_language.startswith("en-"):
            raise GlossaryViewerError(f"{field} must not redefine canonical English labels")
        if not isinstance(raw, dict) or set(raw) != {"term", "aliases"}:
            raise GlossaryViewerError(f"{field}.{language} must contain exactly term and aliases")
        result[canonical_language] = {"term": _nonempty_string(raw["term"], f"{field}.{language}.term"), "aliases": _string_array(raw["aliases"], f"{field}.{language}.aliases")}
    return result
...
def _parse_authority(value: Any, field: str) -> dict[str, Any]:
    ...
    if kind not in AUTHORITY_KINDS:
        raise GlossaryViewerError(f"{field}.kind must be normative, upstream, or conventional")
    ...
    for index, raw in enumerate(raw_sources):
        ...
        source = {"title": _nonempty_string(raw["title"], f"{source_field}.title"), "url": _validate_https_url(raw["url"], f"{source_field}.url")}
        for optional in ("version", "locator"):
            if optional in raw:
                source[optional] = _nonempty_string(raw[optional], f"{source_field}.{optional}")
...
def _parse_term(raw: Any, index: int) -> dict[str, Any]:
    ...
    required = {"id", "term", "aliases", "origin", "provider", "source_path", "source_revision"}
    ...
    result: dict[str, Any] = {"id": term_id, "term": _nonempty_string(raw["term"], f"{field}.term"), "aliases": _string_array(raw["aliases"], f"{field}.aliases"), "origin": origin, "provider": provider, "source_path": _validate_source_path(raw["source_path"], f"{field}.source_path"), "source_revision": revision}
    ...
    if "localized_labels" in raw:
        result["localized_labels"] = _parse_localized_labels(raw["localized_labels"], f"{field}.localized_labels")
    if "related_terms" in raw:
        ...
        for related_id in related:
            if TERM_ID.fullmatch(related_id) is None:
                raise GlossaryViewerError(f"{field}.related_terms contains an invalid term ID: {related_id}")
            if related_id == term_id:
                raise GlossaryViewerError(f"{field}.related_terms must not reference the term itself")
    ...
    if origin == "repository":
        if REPOSITORY_TERM_ID.fullmatch(term_id) is None:
            raise GlossaryViewerError(f"{field}.id must start with templates- for repository terms")
        if "definition" not in result:
            raise GlossaryViewerError(f"{field}.definition is required")
        if "authority" in result:
            raise GlossaryViewerError(f"{field}.authority is not allowed for repository terms")
    else:
        if EXTERNAL_TERM_ID.fullmatch(term_id) is None:
            raise GlossaryViewerError(f"{field}.id must use external-<domain>-<slug> for external terms")
        if "summary" not in result or "authority" not in result:
            raise GlossaryViewerError(f"{field} external terms require summary and authority")
        if "definition" in result:
            raise GlossaryViewerError(f"{field}.definition is not allowed for external terms")
    return result
...
def load_model(path: Path) -> dict[str, Any]:
    ...
    def unique_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, value in pairs:
            if key in result:
                raise GlossaryViewerError(f"input contains duplicate JSON member: {key}")
            result[key] = value
        return result
    try:
        value = json.loads(path.read_text(encoding="utf-8"), object_pairs_hook=unique_object)
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise GlossaryViewerError(f"unable to read glossary JSON {path}: {exc}") from exc
    ...
    for term in terms:
        for related in term.get("related_terms", []):
            if related not in known:
                raise GlossaryViewerError(f"term {term['id']} references unknown related term {related}")
...
def source_url(repository: str, term: dict[str, Any]) -> str:
    ...
    owner, name = repository.split("/", 1)
    source = "/".join(quote(part, safe="") for part in term["source_path"].split("/"))
    return f"https://github.com/{quote(owner, safe='')}/{quote(name, safe='')}/blob/{term['source_revision']}/{source}"


def _join_labels(values: list[str], language: str | None = None) -> str:
    attrs = f' lang="{html.escape(language, quote=True)}"' if language else ""
    return ", ".join(f"<span{attrs}>{html.escape(value)}</span>" for value in values)
...
def render_localized_labels(term: dict[str, Any]) -> str:
    ...
    for language, labels in term.get("localized_labels", {}).items():
        ...
        alias_html = "" if not aliases else '<span class="localized-aliases">aliases: ' + _join_labels(aliases, language) + "</span>"
        rows.append('<p class="localized-label">' f'<span class="language-tag">{html.escape(language)}</span> ' f'<strong lang="{html.escape(language, quote=True)}">{html.escape(labels["term"])}</strong>{alias_html}</p>')
    return "".join(rows)
...
def render_authority(term: dict[str, Any]) -> str:
    ...
    for source in authority["sources"]:
        ...
        items.append('<li><a href="' + html.escape(source["url"], quote=True) + '" target="_blank" rel="noopener">' + html.escape(source["title"]) + "</a>" + suffix + "</li>")
    return '<div class="authority"><p><strong>External authority:</strong> ' f'<span class="badge">{html.escape(authority["kind"])}</span></p><ul>{"".join(items)}</ul></div>'
...
def render_related(term: dict[str, Any], by_id: dict[str, dict[str, Any]]) -> str:
    ...
    if not related:
        return ""
    links = ", ".join(f'<a href="#{html.escape(term_id, quote=True)}">{html.escape(by_id[term_id]["term"])}</a>' for term_id in related)
    return f'<p class="term-related"><strong>Related:</strong> {links}</p>'


def render_term(repository: str, term: dict[str, Any], by_id: dict[str, dict[str, Any]]) -> str:
    aliases_html = "" if not term["aliases"] else '<p class="term-aliases"><strong>English aliases:</strong> ' + _join_labels(term["aliases"]) + "</p>"
    if term["origin"] == "repository":
        explanation_html = '<p class="term-definition">' + html.escape(term["definition"]) + "</p>"
        if "summary" in term:
            explanation_html += '<p class="term-summary"><strong>Summary:</strong> ' + html.escape(term["summary"]) + "</p>"
    else:
        explanation_html = '<p class="term-definition">' + html.escape(term["summary"]) + "</p>"
    usage = "" if "repository_usage" not in term else '<p class="repository-usage"><strong>Repository usage:</strong> ' + html.escape(term["repository_usage"]) + "</p>"
    immutable = source_url(repository, term)
    origin_label = "Templates-defined" if term["origin"] == "repository" else "External"
    return f'''<article class="term-card" id="{html.escape(term['id'], quote=True)}"><header class="term-header"><div><h3>{html.escape(term['term'])}</h3>{render_localized_labels(term)}</div><div class="term-badges"><span class="badge">{origin_label}</span><span class="badge">{PROVIDER_LABELS[term['provider']]}</span></div></header>{explanation_html}{aliases_html}{usage}{render_related(term, by_id)}{render_authority(term)}<details class="provenance"><summary>Source and stable identity</summary><p><strong>Term ID:</strong> <code>{html.escape(term['id'])}</code></p><p><strong>Owner/curator:</strong> <code>{html.escape(term['provider'])}</code></p><p><strong>Source:</strong> <a href="{html.escape(immutable, quote=True)}" target="_blank" rel="noopener"><code>{html.escape(term['source_path'])}</code> at {html.escape(term['source_revision'][:12])}</a></p></details></article>'''


def page_shell(title: str, body: str) -> str:
    return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; manifest-src 'self'; base-uri 'none'; form-action 'none'"><title>{html.escape(title)} · Templates Documentation Portal</title><style>:root{{color-scheme:light dark;font-family:system-ui,sans-serif}}*{{box-sizing:border-box}}html{{scroll-behavior:smooth}}body{{margin:0;background:Canvas;color:CanvasText}}a{{color:LinkText}}code{{font-family:ui-monospace,SFMono-Regular,Consolas,monospace;overflow-wrap:anywhere}}main{{max-width:76rem;margin:0 auto;padding:1.5rem 1rem 4rem}}.portal-link{{display:inline-block;margin-bottom:1rem;font-size:.9rem}}.glossary-path{{margin:0 0 1rem;padding:.5rem .7rem;border:1px solid color-mix(in srgb,CanvasText 16%,transparent);border-radius:.55rem;background:color-mix(in srgb,CanvasText 3%,Canvas);font-size:.86rem}}.glossary-path-label{{font-weight:650;margin-right:.25rem}}.eyebrow{{margin-bottom:.35rem;font-size:.78rem;text-transform:uppercase;letter-spacing:.08em;opacity:.65}}h1{{margin:0;font-size:clamp(2rem,7vw,3.4rem);letter-spacing:-.035em}}.lead{{max-width:58rem;font-size:1.05rem;line-height:1.65}}.summary-grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(12rem,1fr));gap:.65rem;margin:1.25rem 0}}.summary-card{{padding:.85rem 1rem;border:1px solid color-mix(in srgb,CanvasText 16%,transparent);border-radius:.7rem;background:color-mix(in srgb,CanvasText 2.5%,Canvas)}}.summary-card strong{{display:block;font-size:1.4rem}}.jump-links{{display:flex;flex-wrap:wrap;gap:.5rem;margin:1rem 0 2rem}}.jump-links a{{display:inline-block;border:1px solid color-mix(in srgb,CanvasText 18%,transparent);border-radius:999px;padding:.35rem .65rem;text-decoration:none}}.glossary-section{{margin-top:2.5rem}}.section-intro{{max-width:58rem;margin-top:0;opacity:.8;line-height:1.55}}.provider-section{{margin-top:1.7rem}}.term-list{{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,25rem),1fr));gap:.85rem}}.term-card{{scroll-margin-top:1rem;border:1px solid color-mix(in srgb,CanvasText 16%,transparent);border-radius:.75rem;padding:1rem;background:color-mix(in srgb,CanvasText 2%,Canvas)}}.term-header{{display:flex;align-items:flex-start;justify-content:space-between;gap:.75rem}}.term-header h3{{margin:0;font-size:1.18rem}}.term-badges{{display:flex;flex-wrap:wrap;justify-content:flex-end;gap:.3rem}}.badge{{display:inline-block;border:1px solid color-mix(in srgb,CanvasText 20%,transparent);border-radius:999px;padding:.08rem .45rem;font-size:.72rem;white-space:nowrap}}.localized-label{{margin:.28rem 0 0;font-size:.9rem}}.language-tag{{display:inline-block;margin-right:.25rem;font:.7rem/1.5 ui-monospace,monospace;opacity:.6}}.localized-aliases{{display:block;margin-top:.15rem;font-size:.78rem;opacity:.72}}.term-definition{{margin:.8rem 0;line-height:1.55}}.term-summary{{margin:-.25rem 0 .8rem;font-size:.9rem;line-height:1.5;opacity:.86}}.term-aliases,.repository-usage,.term-related{{margin:.5rem 0;font-size:.88rem;line-height:1.45}}.authority{{margin-top:.75rem;padding-top:.6rem;border-top:1px solid color-mix(in srgb,CanvasText 12%,transparent);font-size:.86rem}}.authority ul{{margin:.4rem 0 0;padding-left:1.2rem}}.provenance{{margin-top:.8rem;font-size:.78rem;opacity:.8}}@media(max-width:600px){{main{{padding:1rem .75rem 3rem}}.term-header{{display:block}}.term-badges{{justify-content:flex-start;margin-top:.55rem}}.term-list{{grid-template-columns:1fr}}}}</style></head><body><main><a class="portal-link" href="/">← Documentation portal</a><p class="glossary-path"><span class="glossary-path-label">Page path:</span> <code>/glossary/</code></p>{body}</main></body></html>'''
...
def render(model: dict[str, Any]) -> str:
    ...
    japanese_terms = sum(any(language.casefold() == "ja" or language.casefold().startswith("ja-") for language in term.get("localized_labels", {})) for term in terms)
    provider_sections = []
    for provider in PROVIDER_ORDER:
        owned = sorted((term for term in repository_terms if term["provider"] == provider), key=lambda term: (term["term"].casefold(), term["id"]))
        if owned:
            cards = "".join(render_term(model["repository"], term, by_id) for term in owned)
            provider_sections.append(f'<section class="provider-section" id="provider-{provider}"><h3>{html.escape(PROVIDER_LABELS[provider])}</h3><div class="term-list">{cards}</div></section>')
    external_cards = "".join(render_term(model["repository"], term, by_id) for term in sorted(external_terms, key=lambda term: (term["term"].casefold(), term["id"])))
...
def generate(input_path: Path, output_path: Path) -> None:
    ...
    if parent.is_symlink() or not parent.is_dir():
        raise GlossaryViewerError("output parent must be an existing regular directory")
