...
def load_source_state() -> SourceState:
    ...
    actual_components = sorted(
        path.name
        for path in (SOURCE_ROOT / "components").iterdir()
        if path.is_dir()
    )
...
def _validate_source_graph(
    ...
    def visit(component_id: str) -> None:
        ...
        if descriptor["kind"] in {"capability", "lifecycle"} and any(
            ...
            raise CompositionError("GENERIC_ARTIFACT_DEPENDENCY", f"generic component {component_id} references an artifact component")
    ...
    for recipe_id, recipe in recipes.items():
        ...
        for component_id in set().union(*groups):
            if component_id not in components or components[component_id]["kind"] == "artifact":
                raise CompositionError("INVALID_RECIPE", f"recipe {recipe_id} references invalid selectable component {component_id}")
...
def resolve_configuration(
    ...
    if unknown_include:
        raise CompositionError("COMPONENT_NOT_EXPOSED", f"recipe {recipe_id} does not expose included components: {sorted(unknown_include)}")
    if unknown_exclude:
        raise CompositionError("COMPONENT_NOT_EXPOSED", f"recipe {recipe_id} does not expose excluded components: {sorted(unknown_exclude)}")
    required_excluded = required & exclude
    if required_excluded:
        raise CompositionError("REQUIRED_COMPONENT_EXCLUDED", f"cannot exclude recipe-required components: {sorted(required_excluded)}")
    ...
    if excluded_dependencies:
        raise CompositionError("EXCLUDED_DEPENDENCY", f"excluded components are required by the resolved closure: {sorted(excluded_dependencies)}")

    for component_id in sorted(selected):
        active_conflicts = set(state.components[component_id]["conflicts"]) & selected
        if active_conflicts:
            raise CompositionError("COMPONENT_CONFLICT", f"{component_id} conflicts with selected components {sorted(active_conflicts)}")

    parameter_components = set(config.get("parameters", {}))
    unresolved_parameters = parameter_components - selected
    if unresolved_parameters:
        raise CompositionError("PARAMETER_COMPONENT_UNRESOLVED", f"parameters target unresolved components: {sorted(unresolved_parameters)}")
    ...
    for component_id in selected:
        for registration in state.components[component_id].get("contract_registrations", []):
            ...
            if registration["document"] in documents:
                raise CompositionError("DUPLICATE_CONTRACT_REGISTRATION", f"duplicate contract document: {registration['document']}")
            if registration["schema"] in schemas:
                raise CompositionError("DUPLICATE_CONTRACT_REGISTRATION", f"duplicate contract schema: {registration['schema']}")
        ...
        for declaration in descriptor["materials"]:
            if "source" in declaration:
                ...
                if source.is_symlink() or not source.is_file():
                    raise CompositionError("INVALID_SOURCE_MATERIAL", f"source material is missing, non-regular, or a symlink: {component_id}/{declaration['source']}")
                try:
...
def _validate_material_destinations(materials: list[Material]) -> None:
    ...
    for material in materials:
        parts = _normalized_parts(material.destination)
        if parts == lock_parts[: len(parts)] or lock_parts == parts[: len(lock_parts)]:
            raise CompositionError("RESERVED_DESTINATION", f"material conflicts with reserved lock path: {material.destination}")
        normalized.append((material, parts))
    for index, (left_material, left) in enumerate(normalized):
        for right_material, right in normalized[index + 1 :]:
            if left == right:
                raise CompositionError("DESTINATION_COLLISION", f"material destinations collide: {left_material.destination}, {right_material.destination}")
            if left == right[: len(left)] or right == left[: len(right)]:
                raise CompositionError("DESTINATION_COLLISION", f"material file/directory paths conflict: {left_material.destination}, {right_material.destination}")
        ...
        for actual_text, actual, kind in existing:
            if actual == planned and actual_text != material.destination:
                material_conflicts.append(
                    f"portable case collision with existing {kind}: {actual_text}"
                )
            elif actual == planned:
                continue
                ...
                if kind != "directory":
                    material_conflicts.append(
                        f"planned parent path is existing {kind}: {actual_text}"
                    )
            elif planned == actual[: len(planned)] and len(planned) < len(actual):
                material_conflicts.append(
                    f"planned file would contain existing path: {actual_text}"
                )
        if exact.is_symlink():
...
def _write_no_overwrite(destination: Path, data: bytes) -> None:
...
def apply_plan(
    ...
    for action in actions:
        ...
        _write_no_overwrite(target / material.destination, material.data)
    lock_bytes = (json.dumps(lock, ensure_ascii=False, indent=2) + "\n").encode("utf-8")
    _write_no_overwrite(target / LOCK_RELATIVE, lock_bytes)


def _load_consumer_validator_module_text(target: Path) -> Path:
    validator = target / ".template-composition/validate_composition.py"
    if not validator.is_file() or validator.is_symlink():
        raise CompositionError("VALIDATOR_MISSING", "materialized composition validator is missing or unsafe")
    return validator


def validate_consumer_with_materialized_validator(target: Path) -> tuple[bool, list[str]]:
    validator = _load_consumer_validator_module_text(target)
    result = subprocess.run(
    ...
    if not errors:
        errors = [result.stdout.strip() or f"validator exited with status {result.returncode}"]
    return False, errors
...
def command_inspect(target: Path) -> tuple[int, dict[str, Any]]:
    ...
    try:
        valid, errors = validate_consumer_with_materialized_validator(target)
...
def command_plan(config_path: Path, target: Path) -> tuple[int, dict[str, Any]]:
    ...
    return (0 if not conflicts else 2), result
