...
def load_source_state() -> SourceState:
    ...
    if component_ids != sorted(component_ids) or len(component_ids) != len(
        ...
        raise CompositionError(
            "INVALID_CATALOG",
            "catalog component IDs must be unique and lexically ordered",
        )
    if recipe_ids != sorted(recipe_ids) or len(recipe_ids) != len(set(recipe_ids)):
        raise CompositionError(
            "INVALID_CATALOG",
            "catalog recipe IDs must be unique and lexically ordered",
        )
    ...
    actual_recipes = sorted(
        path.stem for path in (SOURCE_ROOT / "recipes").glob("*.json")
    )
    if actual_components != component_ids:
        raise CompositionError(
            "CATALOG_NOT_CLOSED",
            "catalog component inventory does not match components/",
        )
    if actual_recipes != recipe_ids:
        raise CompositionError(
            "CATALOG_NOT_CLOSED",
            "catalog recipe inventory does not match recipes/",
        )
    ...
    for component_id in component_ids:
        ...
        if descriptor["id"] != component_id:
            raise CompositionError(
                "INVALID_COMPONENT",
                f"descriptor identity mismatch for {component_id}",
            )
    ...
    for recipe_id in recipe_ids:
        ...
        if recipe["id"] != recipe_id:
            raise CompositionError(
                "INVALID_RECIPE",
                f"recipe identity mismatch for {recipe_id}",
            )
...
def _validate_source_graph(
    ...
    def visit(component_id: str) -> None:
        ...
        if component_id in visiting:
            raise CompositionError(
                "DEPENDENCY_CYCLE",
                f"component dependency cycle at {component_id}",
            )
        visiting.add(component_id)
        descriptor = components[component_id]
        if component_id in descriptor["requires"] or component_id in descriptor[
            "conflicts"
        ]:
            raise CompositionError(
        ...
        if overlap:
            raise CompositionError(
                "INVALID_COMPONENT",
                f"{component_id} both requires and conflicts with {sorted(overlap)}",
            )
        for reference in descriptor["requires"] + descriptor["conflicts"]:
            if reference not in ids:
                raise CompositionError(
                    "UNKNOWN_COMPONENT",
                    f"{component_id} references missing component {reference}",
                )
    ...
    for recipe_id, recipe in recipes.items():
        ...
        if artifact not in components or components[artifact]["kind"] != "artifact":
            raise CompositionError(
                "INVALID_RECIPE",
                f"recipe {recipe_id} has unknown artifact {artifact}",
            )
        groups = [
        ...
        if groups[0] & groups[1] or groups[0] & groups[2] or groups[1] & groups[2]:
            raise CompositionError(
                "INVALID_RECIPE",
                f"recipe {recipe_id} selection classes overlap",
            )
        for component_id in set().union(*groups):
            if (
                component_id not in components
                or components[component_id]["kind"] == "artifact"
            ):
                raise CompositionError(
                    "INVALID_RECIPE",
...
def resolve_configuration(
    ...
    if include & exclude:
        raise CompositionError(
            "SELECTION_OVERLAP",
            f"include/exclude overlap: {sorted(include & exclude)}",
        )
    ...
    if excluded_dependencies:
        raise CompositionError(
            "EXCLUDED_DEPENDENCY",
            "excluded components are required by the resolved closure: "
            f"{sorted(excluded_dependencies)}",
        )
...
def _render_contract_manifest(state: SourceState, selected: list[str]) -> bytes:
    ...
    for component_id in selected:
        for registration in state.components[component_id].get(
            "contract_registrations", []
        ):
            if registration["id"] in ids:
                raise CompositionError(
                    "DUPLICATE_CONTRACT_REGISTRATION",
                    f"duplicate contract id: {registration['id']}",
                )
        ...
        for declaration in descriptor["materials"]:
            if "source" in declaration:
                ...
                try:
                    ...
                    raise CompositionError(
                        "READ_FAILED",
                        f"cannot read source material {source}: {exc}",
                    ) from exc
            else:
                ...
                if generator not in SUPPORTED_GENERATORS:
                    raise CompositionError(
                        "UNKNOWN_GENERATOR",
                        f"unsupported generated material handler: {generator}",
                    )
    ...
    for material in materials:
        ...
        if parts == lock_parts[: len(parts)] or lock_parts == parts[: len(lock_parts)]:
            raise CompositionError(
                ...
                f"material conflicts with reserved lock path: {material.destination}",
            )
        ...
        for right_material, right in normalized[index + 1 :]:
            if left == right:
                raise CompositionError(
                    ...
                    "material destinations collide: "
                    f"{left_material.destination}, {right_material.destination}",
                )
            if left == right[: len(left)] or right == left[: len(right)]:
                raise CompositionError(
                    "DESTINATION_COLLISION",
                    "material file/directory paths conflict: "
                    f"{left_material.destination}, {right_material.destination}",
                )
...
def plan_target(
        ...
        if exact.is_symlink():
            ...
            if not exact.is_file():
                ...
                try:
                    ...
                    material_conflicts.append(
                        f"cannot read existing destination: {exc}"
                    )
                else:
    ...
    return (
        sorted(actions, key=lambda entry: entry["destination"]),
...
def _write_no_overwrite(target: Path, destination: Path, data: bytes) -> None:
    ...
    if not _parent_chain_is_safe(target, destination):
        raise CompositionError(
            "WRITE_CONFLICT",
            f"parent path changed unsafely while applying {destination}",
        )
    ...
    try:
        ...
        try:
            ...
            raise CompositionError(
                "WRITE_CONFLICT",
                f"destination appeared during apply: {destination}",
            ) from exc
        except OSError as exc:
            raise CompositionError(
...
def apply_plan(
    ...
    material_by_destination = {
        material.destination: material for material in materials
    }
...
def validate_consumer_with_source_validator(
    ...
    if not errors:
        errors = [
            result.stdout.strip()
            or f"source validator exited with status {result.returncode}"
        ]
    return False, errors
...
def command_apply(
    ...
    return 0, {
        ...
        "adopted": [
            entry["destination"]
            for entry in actions
            if entry["action"] == "adopt-identical"
        ],
        "lock": LOCK_RELATIVE,
    }
