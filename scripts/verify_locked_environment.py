...
DEFAULT_LOCK = ROOT / "requirements-dev.lock"
...
def compare_distribution_sets(
    ...
    if unexpected:
        ...
        errors.append("unexpected distributions outside the lock: " + rendered)
...
def main() -> int:
    try:
        ...
        print(f"Locked environment verification failed: {exc}", file=sys.stderr)
        return 1

    if errors:
        for error in errors:
            print(f"Locked environment verification failed: {error}", file=sys.stderr)
        return 1

    print("Installed distribution set matches requirements-dev.lock.")
    return 0
