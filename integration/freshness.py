#!/usr/bin/env python3
"""Classify a reviewed provider lock against an exact current snapshot."""
...
from publication_bundle.contract import SHA as FULL_COMMIT_PATTERN
...
def validate_revision(label: str, value: str) -> str:
    if not FULL_COMMIT_PATTERN.fullmatch(value):
        raise PublicationFreshnessError(
            f"{label} provider revision must be a full lowercase commit SHA"
        )
    return value
