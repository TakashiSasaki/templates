...
from publication_bundle.graph import GRAPH_SCHEMA_VERSION, find_cycle_edges, graph_diagnostics
...
from publication_bundle.url_contract import (
    CONTEXTUAL_JOINERS,
    FORBIDDEN_DOMAIN_CHARACTERS,
    IPV4_NUMBER,
    IndexNavigationError,
    NON_MARKDOWN_LINE_SEPARATORS,
    canonicalize_whatwg_domain,
    contains_disallowed_control,
    contains_forbidden_domain_codepoint,
    contextual_joiners_are_valid,
    decode_external_hostname,
    ipv4_ends_in_number,
    parse_ipv4_number,
    remove_optional_terminal_empty_part,
    validate_ascii_punycode_labels,
    validate_browser_ipv4_candidate,
    validate_external_location,
    validate_whatwg_unicode_labels,
)
...
def parse_index(text: str, path: str) -> ParsedIndex:
    ...
    for line_number, raw_line in enumerate(normalized_text.split("\n"), start=1):
        if line_number == 1 and raw_line == "<!-- generated by maintain-progressive-discovery -->":
            continue
