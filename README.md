...
The clean-room generated-repository class is template-maintainer-only. When this test file is retained after a generated repository switches its source evidence document to `mode: product`, that class reports skipped and a separate scope regression verifies the boundary.
...
The fixture registers one reviewed product proof command and one selected release gate. The test harness invokes the known proof script directly with a fixed argument vector; it does not parse or execute arbitrary command strings. The script verifies 52 positive and negative outcomes, after which the harness runs all six retained validator entry points from the generated repository root.

For the first seven negative generated copies, the harness invokes the copied standalone implementation-evidence validator, requires a nonzero exit, and matches the expected stderr diagnostic. The false-proof case invokes the copied reviewed product proof. Coverage includes template residue, missing targets, unverified boundaries, unknown or unused commands, unused gates, release-gate closure gaps, and false proof results. The template source remains in `mode: template` before and after every fixture run. See [`docs/architecture/generated-repository-conformance.md`](docs/architecture/generated-repository-conformance.md).
