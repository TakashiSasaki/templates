...
def render_snapshot(*,bundle,site_root,output,identity,site_revision,parent_identity,parent_directory,original_bundle,original_site,public_url,deployment_timestamp):
    with tempfile.TemporaryDirectory(prefix='site-render-') as temporary:
        ...
        run(site_root,'finalize_translation_reader.py','--site-root',site,'--translation-map',build/'translation-publication.json','--canonical-url',public_url,'--availability',build/'translation-coverage.json','--availability',build/'site-translation-coverage.json')
