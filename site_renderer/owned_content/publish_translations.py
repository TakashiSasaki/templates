...
from site_renderer.owned_content.translation_link_identity import (
...
def _load_records(
    ...
    if set(publications) != {'site'}:
        raise TranslationPublicationError('Site content compiler rejects provider checkouts')
    page_destinations = {
    ...
    for publication, (root, documents, _) in sorted(publications.items()):
        ...
        for entry in manifest.for_surface("reader"):
            ...
            # Site owns these derivatives. Stale remains available with exact
            # availability evidence; the reader adds its non-authoritative warning.
            try:
...
def publish_translations(
    ...
    Site-only availability records distinguish current and stale derivatives
