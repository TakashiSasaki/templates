...
5. Start the unit with `sudo systemctl start text-stats-mcp.service` and require both `systemctl is-active` and `GET http://127.0.0.1:4572/readyz` to succeed before MCP initialization.
6. Invoke `POST http://127.0.0.1:4572/mcp` with the exact Bearer token and the selected MCP revision.
