...
| Graceful shutdown and restart policy | TERM and INT handlers are installed before PID record publication; the record is created exclusively, forced through the same open descriptor to exact mode `0600` independently of the process umask, and verified before its contents are published. Later reads use one nonblocking no-follow descriptor and require a regular service-user-owned mode-0600 file no larger than 4,096 bytes containing PID and Linux process start ticks; `--stop` identity-verifies the record before signaling, and restart is an external operator action. |
...
| Security and deployment smoke tests | Tests cover token-file descriptor validation including writerless FIFO rejection, Bearer authentication, Host and Origin rejection, request bounds and timeout recovery, concurrency, bounded health commands, health isolation, fixed-port collision, PID descriptor and identity validation including writerless FIFO rejection, graceful shutdown, and prompt missing-implementation failure. A dedicated Ruby 3.1 smoke test starts the real service under `umask 0777`, verifies that the PID record is still exact mode `0600`, invokes the documented `--stop` command, and verifies record cleanup. |
...
| `TEXT_STATS_SERVICE_PID_FILE` | NO | Overrides the default `tmp/text-stats-service.pid` lifecycle record; creation forces and verifies exact mode `0600` through the open descriptor regardless of umask, and subsequent reads use nonblocking no-follow semantics with the ownership, type, size, and process-identity checks above. | NO |
