...
| Source layout | `src/text_stats.rb` contains deterministic domain logic plus the bounded lifecycle-probe and secure PID-record writer modules; `service/server.rb` contains the authenticated HTTP adapter, listener, routing, and lifecycle command. |
...
| Graceful shutdown and restart policy | TERM and INT handlers are installed before PID record publication. The record is built in a same-directory exclusive no-follow staging file, forced to exact mode `0600` independently of umask, fully written, flushed, synchronized, and verified through the same descriptor. A no-replace hard link then publishes the complete inode atomically at the configured PID pathname; incomplete writes and publication failures remove the staging entry, and a post-link verification failure removes only the inode published by that call. Later reads use one nonblocking no-follow descriptor and require a regular service-user-owned mode-0600 file no larger than 4,096 bytes containing PID and Linux process start ticks; `--stop` identity-verifies the record before signaling, and restart is an external operator action. |
...
| Security and deployment smoke tests | Tests cover token-file descriptor validation including writerless FIFO rejection, Bearer authentication, Host and Origin rejection, request bounds and timeout recovery, concurrency, bounded health commands, health isolation, fixed-port collision, PID descriptor and identity validation including writerless FIFO rejection, graceful shutdown, and prompt missing-implementation failure. Dedicated Ruby 3.1 smoke tests start the real service under `umask 0777`, verify exact PID-record mode `0600`, invoke the documented `--stop` command, verify record cleanup, hold atomic publication immediately before the final link to prove the configured pathname is still absent while staging content is complete, and inject a partial write to prove neither a final nor staging record remains. |
...
| `TEXT_STATS_SERVICE_PID_FILE` | NO | Overrides the default `tmp/text-stats-service.pid` lifecycle record. The configured pathname is published only after a complete same-directory staging record has been mode-forced, synchronized, and descriptor-verified; publication refuses replacement. Subsequent reads use nonblocking no-follow semantics with the ownership, mode, type, size, and process-identity checks above. | NO |
