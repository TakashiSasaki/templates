...
| Security and deployment smoke tests | Tests cover token-file descriptor validation including writerless FIFO rejection, Bearer authentication, Host and Origin rejection, request bounds and timeout recovery, concurrency, bounded health commands, health isolation, fixed-port collision, PID descriptor and identity validation including writerless FIFO rejection, graceful shutdown, and prompt missing-implementation failure. Dedicated Ruby 3.1 smoke tests start the real service under `umask 0777` from a fresh runtime directory with no PID override, verify creation of the default `tmp` directory at exact mode `0700` and its PID record at exact mode `0600`, invoke the documented `--stop` command, verify record cleanup, hold atomic publication immediately before the final link to prove the configured pathname is still absent while staging content is complete, and inject a partial write to prove neither a final nor staging record remains. |
