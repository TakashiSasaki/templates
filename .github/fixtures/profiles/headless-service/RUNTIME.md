...
| Security and deployment smoke tests | Tests cover token-file descriptor validation including writerless FIFO rejection, Bearer authentication, Host and Origin rejection, request bounds and timeout recovery, concurrency, bounded health commands, health isolation, fixed-port collision, PID descriptor and identity validation including writerless FIFO rejection, graceful shutdown, and prompt missing-implementation failure |
