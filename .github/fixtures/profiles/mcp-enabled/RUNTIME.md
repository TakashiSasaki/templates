...
| Source layout | `src/text_stats.rb` contains deterministic operation logic; `mcp/server_factory.rb` owns the shared tool and server definition; `mcp/server.rb` and `mcp/http_server.rb` are thin stdio and Streamable HTTP adapters; `mcp/service_manager.rb` is an optional local lifecycle controller; `tests/` contains transport, boundary, lifecycle, client, and managed-deployment tests. |
| Supported operating systems | Linux with CRuby 3.1 or newer and `/proc` process identity support for the managed lifecycle variant |
...
| Readiness check | Unauthenticated `GET /readyz` returns `{"status":"ready"}` only after the listener is serving; it still applies the same per-request canonical Host and Origin gate and remains independent of MCP request failures and session capacity |
| Liveness check | Unauthenticated `GET /livez` returns `{"status":"live"}` while the HTTP event loop is serving. The managed `live` command additionally verifies the PID and Linux process-start identity before probing the endpoint. |
...
| Shutdown/restart policy | Manual foreground mode records TERM or INT even before the server callback attaches, shuts down WEBrick once available, closes SDK sessions, releases the port, and can restart on the same port. Optional managed mode uses an identity-verified mode-0600 PID record, bounded readiness, explicit start/stop/restart, TERM followed by KILL after fixed grace periods, and safe stale-record removal. No automatic restart is claimed. |
...
| Process topology | One detached local process group running the same `mcp/http_server.rb` adapter |
| Start policy | Validate the external token file before process creation, reject an already-live identity, safely remove only a stale verified PID record, create a mode-0600 log, spawn the fixed Ruby entry point, publish a mode-0600 PID/start-tick record, and require readiness within eight seconds |
| Stop policy | Verify PID plus Linux `/proc` start ticks before signaling the process group; send TERM, wait two seconds, send KILL when still live, wait one additional second, and remove only the unchanged verified record |
| Restart policy | Complete bounded stop before a new start; no overlap, zero-downtime handoff, or automatic restart is claimed |
| Stale-process policy | A missing process, zombie, or start-tick mismatch is stale. The controller removes the unchanged safe record without signaling an unrelated process. A malformed, symlinked, wrong-owner, non-regular, oversized, or overly permissive record is rejected rather than repaired or followed. |
| Secret boundary | Managed startup requires `TEXT_STATS_MCP_HTTP_TOKEN_FILE`; the secret value is never placed in argv, PID metadata, stdout, stderr, or the managed log. Direct environment-token mode remains supported only for explicit foreground launch. |
| Runtime files | Default PID `tmp/text-stats-mcp-http.pid` and log `tmp/text-stats-mcp-http.log`; missing directories are created owner-only, PID publication is no-replace and atomic, and files are mode `0600` |
...
| Smoke and negative tests | Real start/ready/live/restart/stop, stale-record replacement, token permission and symlink rejection before process creation, PID symlink refusal, secret redaction, and TERM-ignoring process-group escalation |
