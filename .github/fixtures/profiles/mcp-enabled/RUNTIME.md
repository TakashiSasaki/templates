...
| Default revision or negotiation mode | Fixed revision `2025-11-25`; any other requested revision is rejected with JSON-RPC invalid params. |
| MCP SDK or protocol library | Official Ruby MCP SDK gem `mcp` |
| SDK version | `1.0.0` |
| Legacy compatibility policy | No protocol-revision fallback is exposed by this fixture. |
...
| Negotiation and compatibility tests | Tests verify exact-revision initialization, revision rejection, tools-only capability advertisement, and continued operation after protocol and tool-validation errors. |
...
| Protocol negotiation/discovery | One `initialize` request for revision `2025-11-25`, followed by `notifications/initialized` and standard `tools/list` discovery |
...
| Cancellation behavior | The only operation is bounded and synchronous; a caller timeout closes stdin and then applies bounded TERM/KILL escalation if the child does not exit. |
