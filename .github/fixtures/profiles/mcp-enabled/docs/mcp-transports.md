...
- `GET /readyz` and `GET /livez` expose minimal separate readiness and liveness states and require no token;
...
Managed startup validates the external secret before process creation, rejects a currently live recorded identity, removes only a safely parsed stale record, creates owner-only runtime paths, starts one process group, publishes an atomic mode-0600 PID plus Linux start-tick record, and requires bounded readiness. Stop and restart verify PID and start ticks before signaling. TERM receives a fixed grace interval before KILL escalation. A missing process, zombie, or start-tick mismatch is stale and is not signaled; malformed, symlinked, non-regular, wrong-owner, oversized, or overly permissive records fail closed. The token value is absent from argv, PID metadata, controller output, and the managed log.
...
The fixture tests actual initialization, discovery, and tool calls over both transports and through the bundled client, and requires equal `structuredContent` for equal input. Managed-mode tests use the same HTTP adapter and add only process, secret, readiness/liveness, stale-record, and shutdown-escalation assertions. Transport-specific status codes, headers, authentication, sessions, framing, and lifecycle remain adapter concerns and do not alter domain semantics.
