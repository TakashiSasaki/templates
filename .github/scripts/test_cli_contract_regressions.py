#!/usr/bin/env python3
"""Run direct Python regression matrices for packaged-CLI contracts."""
...
def main() -> int:
    ...
    for case_name, zero_meaning, nonzero_meaning, expected_success in exit_cases:
        with tempfile.TemporaryDirectory(prefix="python-cli-exit-regression-") as directory:
            root = Path(directory)
            _write_skill(root)
            ...
            failure = _run_validator(
                f"exit-code / {case_name}", EXIT_VALIDATOR, root, expected_success
            )
            if failure:
...
""",
    ...
    for case_name, contract in exit_shape_cases:
        with tempfile.TemporaryDirectory(prefix="python-cli-exit-shape-regression-") as directory:
            root = Path(directory)
            _write_skill(root)
            (root / "CLI_INTERFACE.md").write_text(contract, encoding="utf-8")
            failure = _run_validator(
                f"exit-code / {case_name}", EXIT_VALIDATOR, root, False
            )
            if failure:
                failures.append(failure)
    ...
    structured_cases = [
        ...
        ("tabbed NOT SUPPORTED selector", "option: NOT\tSUPPORTED", "JSON", "contractVersion", False),
        ("spaced SEE DOCUMENTATION selector", "option: SEE   DOCUMENTATION", "JSON", "contractVersion", False),
        ("plain text", "--output text", "plain text only", "contractVersion", False),
        ("tabbed plain text", "--output text", "plain\ttext", "contractVersion", False),
        ...
        ("tabbed NOT APPLICABLE format", "--json", "NOT\tAPPLICABLE", "contractVersion", False),
        ("missing version field", "--json", "JSON", "no version field", False),
        ("spaced missing version field", "--json", "JSON", "NO\tVERSION  FIELD", False),
    ...
    for case_name, selector, format_name, version_field, expected_success in structured_cases:
        with tempfile.TemporaryDirectory(prefix="python-cli-structured-regression-") as directory:
            root = Path(directory)
            _write_skill(root)
            (root / "CLI_INTERFACE.md").write_text(
    ...
    total = len(exit_cases) + len(exit_shape_cases) + len(structured_cases)
    print(f"Direct Python CLI contract regression tests passed ({total} cases).")
    return 0
