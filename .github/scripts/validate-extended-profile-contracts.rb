...
if profile_lines.length != 1
  ...
  declarations.each do |declaration|
    ...
    if resolved_value.call(automatic) && !%w[YES NO WITH\ CONDITIONS].include?(automatic.upcase)
  ...
  if selected_profiles.include?("mcp-enabled") && File.file?("INTERFACES.md")
    ...
    variants.each do |variant_name, spec|
      ...
      support = field_value.call(section, "Supported")
      support_token = support&.split(/[;\s]/)&.first&.upcase
      unless %w[YES NO].include?(support_token)
        errors << "MCP interface '#{variant_name}' must set 'Supported:' to YES or NO in INTERFACES.md."
        next
      end

      next unless support_token == "YES"
