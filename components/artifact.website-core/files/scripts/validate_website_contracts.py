...
def valid_https_origin(value: object) -> bool:
    ...
    return all(
        1 <= len(label) <= 63 and HOST_LABEL.fullmatch(label) for label in labels
    )
...
def validate(root: Path) -> list[str]:
    ...
    route_ids = [
        item.get("id") for item in route_list if isinstance(item.get("id"), str)
    ]
    route_set = set(route_ids)
    route_by_id = {
        item["id"]: item for item in route_list if isinstance(item.get("id"), str)
    }
    for duplicate in duplicates(route_ids):
        errors.append(f"duplicate shared Website route id: {duplicate}")
    canonical_paths = [
        item["path"] for item in route_list if isinstance(item.get("path"), str)
    ]
    aliases = [
        alias
        for item in route_list
        for alias in item.get("aliases", [])
        if isinstance(item.get("aliases"), list) and isinstance(alias, str)
    ]
    route_paths = set(canonical_paths) | set(aliases)
    ...
    for collision in sorted(set(canonical_paths) & set(aliases)):
        errors.append(
            f"shared Website route alias collides with canonical route path: {collision}"
        )

    viewports = viewports_doc.get("viewports")
    viewport_list = (
        [item for item in viewports if isinstance(item, dict)]
    ...
    widths = [
        item.get("minWidthPx")
        for item in viewport_list
        if isinstance(item.get("minWidthPx"), int)
        and not isinstance(item.get("minWidthPx"), bool)
    ]
    ...
    if any(current >= following for current, following in zip(widths, widths[1:])):
        errors.append(
            "shared Website viewport breakpoints must be strictly increasing by minWidthPx"
        )
    ...
    page_ids = [
        item.get("id") for item in page_list if isinstance(item.get("id"), str)
    ]
    page_route_ids = [
        item.get("routeId")
        for item in page_list
        if isinstance(item.get("routeId"), str)
    ]
    ...
    for page in page_list:
        page_id = page.get("id")
        if not has_visible_character(page.get("title")):
            errors.append(
                f"Website page {page_id!r} title must contain at least one visible character"
            )
    page_by_id = {
        item["id"]: item for item in page_list if isinstance(item.get("id"), str)
    }
    ...
    if len(home_roles) != 1:
        errors.append(
            f"Website structure must declare exactly one home-role page; found {home_roles}"
        )
    ...
    for page in page_list:
        ...
        if parent is not None and parent not in page_set:
            errors.append(
                f"Website page {page_id!r} references unknown parentPageId {parent!r}"
            )
        if page_id != home_id and parent is None:
            errors.append(
                f"Website non-home page {page_id!r} must declare a parentPageId"
            )
    ...
    if isinstance(navigation, list):
        for page_id in navigation:
            if isinstance(page_id, str) and page_id not in page_set:
                errors.append(
                    f"primary navigation references unknown Website page {page_id!r}"
                )
    ...
    metadata_list = (
        [item for item in metadata_pages if isinstance(item, dict)]
        if isinstance(metadata_pages, list)
        else []
    )
    metadata_ids = [
        item.get("pageId")
        for item in metadata_list
        if isinstance(item.get("pageId"), str)
    ]
    ...
    for item in metadata_list:
        ...
        for field in ("title", "description"):
            if not has_visible_character(item.get(field)):
                errors.append(
                    f"Website document metadata {page_id!r} {field} must contain at least one visible character"
                )
    ...
    if metadata.get("siteSocialPreview") == "none":
        inherited = sorted(
            item.get("pageId")
            for item in metadata_list
            if item.get("socialPreview") == "inherit-site"
        )
        if inherited:
            errors.append(
                f"pages cannot inherit social preview when siteSocialPreview is none: {inherited}"
            )
    ...
    indexable = {
        ...
        for item in metadata_list
        if isinstance(item.get("pageId"), str)
        and item.get("indexability") == "index"
    }
    sitemap = discovery.get("sitemap")
    sitemap_ids = (
        set(sitemap.get("pageIds", []))
        if isinstance(sitemap, dict) and isinstance(sitemap.get("pageIds"), list)
        else set()
    )
    if sitemap_ids != indexable:
        errors.append(
            "Website sitemap pageIds must exactly match indexable document metadata "
            f"pages; expected {sorted(indexable)}, found {sorted(sitemap_ids)}"
        )
    for page_id in sorted(sitemap_ids):
        page = page_by_id.get(page_id)
        route_id = page.get("routeId") if isinstance(page, dict) else None
        route = route_by_id.get(route_id) if isinstance(route_id, str) else None
        if isinstance(route, dict) and route.get("deepLink") is not True:
            errors.append(
                f"Website sitemap page {page_id!r} requires shared route {route_id!r} to support deepLink"
            )
    ...
    if isinstance(robots_path, str) and robots_path == sitemap_path:
        errors.append(
            f"Website robots and sitemap discovery paths must be distinct: {robots_path!r}"
        )
    ...
    for feed in feed_list:
        if feed.get("path") in reserved_paths:
            errors.append(
                "Website feed path collides with robots/sitemap discovery path: "
                f"{feed.get('path')!r}"
            )

    discovery_paths: list[tuple[str, object]] = [
        ("robots", robots_path),
        ("sitemap", sitemap_path),
    ]
    discovery_paths.extend((f"feed {feed.get('id')!r}", feed.get("path")) for feed in feed_list)
    for label, path in discovery_paths:
        if isinstance(path, str) and path in route_paths:
            errors.append(
                f"Website {label} discovery path collides with canonical/alias route path: {path!r}"
            )

    if evidence.get("mode") == "product" and not valid_https_origin(
        discovery.get("canonicalOrigin")
    ):
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print(
        "Website structure, document metadata, discovery, shared-route, and viewport invariants: OK"
    )
    return 0
