...
The component also owns the executable `check-release-readiness` machine operation in `.template-composition/implementation-evidence-actions.json`; lifecycle presentation consumes that registry rather than copying the validator CLI syntax. Its canonical isolated argv invokes the existing semantic validator with `--release-readiness --format json`. The result is validated by `.template-composition/implementation-evidence-release-readiness.schema.json`, reports `ready` or `not-ready`, preserves blocking diagnostics and deferred proof IDs as machine data, and does not mutate product evidence. Human-oriented validator output remains the default when `--format json` is not selected.
