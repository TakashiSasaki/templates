...
- the registered JSON Schema owns document structure and product-mode completeness, including the mandatory non-empty requirement ledger, verified implementation-boundary status, required proof metadata, and at least one selected release gate per product record;
...
Requirement IDs are stable machine-facing identifiers; their descriptions are intentionally opaque to Composition. Lowercase IDs remain valid, and uppercase hyphenated IDs such as `REQ-SEVERITY-BROWSER-FILTER` are also valid for requirement rows. Each requirement must reference at least one implementation-evidence record. The existing record rules then close the graph through an implementation boundary, positive and negative proofs, authoritative commands, and a selected release gate.

The ledger is mandatory in product mode so that a consumer cannot evade traceability by omitting machine-readable intent altogether. Composition still does not infer requirements from prose: the human or coding agent must enumerate the product requirements that are intended to justify completion. Template mode carries `requirements: []` and makes no product claims.
...
A requirement may declare `requiredPositiveProofKinds` when the caller-visible behavior needs a minimum execution class. The validator checks only the declared proof-kind edge. For example, a CLI requirement can require `integration-test`, while a browser interaction requirement can require `end-to-end-test` or `accessibility-test`. This prevents a static `inspection` proof from satisfying a caller-visible interactive requirement without making Composition infer product semantics from requirement prose.
