...
A command harness is a repository-relative file, not arbitrary prose. Normal consumer validation checks that the file exists. This does not prove that the file's implementation is semantically honest; it removes the weaker failure mode where a high-strength proof kind is attached to no identifiable executable harness. When the release lifecycle is selected, `lifecycle.release-execution` additionally owns the fixed-argv command binding and revision-bound release execution/provenance owns the actual execution result.
