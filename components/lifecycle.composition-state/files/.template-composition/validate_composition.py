...
COMPONENT_RE = re.compile(r"^(foundation|artifact|capability|lifecycle|topology|workspace)\.[a-z][a-z0-9]*(?:-[a-z0-9]+)*$")
SELECTABLE_COMPONENT_RE = re.compile(r"^(capability|lifecycle|topology|workspace)\.[a-z][a-z0-9]*(?:-[a-z0-9]+)*$")
PARAMETER_COMPONENT_RE = re.compile(r"^(artifact|capability|lifecycle|topology|workspace)\.[a-z][a-z0-9]*(?:-[a-z0-9]+)*$")
...
def validate_lock_shape(lock: Any) -> list[str]:
    ...
    if not isinstance(resolved, list) or not resolved:
        ...
        if sum(component_id.startswith("workspace.") for component_id in resolved_ids) > 1:
            errors.append("composition lock must resolve at most one workspace component")
