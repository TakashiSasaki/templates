...
def main() -> int:
    ...
    for command in implementation["commands"]:
        ...
        if completed.returncode != 0:
            print(
                f"release command {command_id} failed with exit code {completed.returncode}; canonical release evidence was not changed",
                file=sys.stderr,
            )
            return 1
        command_results.append(
            {
                ...
                "resultLocator": (
                    f"contracts/release-evidence.json#/commandResults/{len(command_results)}"
                ),
            }
        )
    ...
    provenance_locator = (
        args.provenance_locator or "contracts/release-evidence.json"
    )
    gate_results = [
        {
            "gateId": gate["id"],
            "status": "passed",
            "resultLocator": (
                f"contracts/release-evidence.json#/gateResults/{index}"
            ),
        }
