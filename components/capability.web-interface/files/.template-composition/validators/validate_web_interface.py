...
def validate(root: Path) -> list[str]:
    ...
    if interface_mode == "template":
        if evidence_mode == "product":
            return [
                "capability.web-interface is selected but contracts/web-interface.json "
                "remains in template mode while product implementation evidence is active; "
                "either remove capability.web-interface from Composition intent or declare "
                "the Web interface contract in product mode and add endpoint evidence"
            ]
        return []
    ...
    if evidence_mode != "product":
        return [
            "product Web interface contract requires product implementation evidence; "
            "switch contracts/implementation-evidence.json to product mode and prove each "
            "declared endpoint"
        ]
    ...
    for endpoint in endpoints:
        ...
        if endpoint_kind not in {"browser-page", "backend-api", "health"}:
            errors.append(
                f"Web interface endpoint {endpoint_id!r} has unsupported kind "
                f"{endpoint_kind!r}"
            )
    ...
    for duplicate, count in sorted(Counter(address_keys).items()):
        if count > 1:
            errors.append(
                f"duplicate Web interface endpoint address: {duplicate[0]} {duplicate[1]}"
            )
    ...
    for key, matching in sorted(
        ...
        if key in expected and len(matching) != 1:
            errors.append(
                f"Web interface implementation-evidence target {key} must have exactly "
                "one record"
            )
    ...
    for key in sorted(expected & actual, key=str):
        ...
        if not isinstance(record_id, str):
            errors.append(
                f"Web interface target {key} must have a text implementation-evidence "
                "record id"
            )
            continue
        ...
        for field, polarity in (
            ...
            if proof_kinds(record, field).isdisjoint(required):
                errors.append(
                    f"Web interface endpoint {endpoint_id!r} ({endpoint_kind}) requires "
                    f"at least one {polarity} {label} proof kind ({allowed}); static "
                    "inspection or unit-only proof is insufficient"
                )

        linked = requirement_refs.get(record_id, [])
        if not linked:
            errors.append(
                f"Web interface record {record_id!r} must be linked from at least one "
                "product requirement"
            )
            continue
        if not any(
            ...
            errors.append(
                f"Web interface record {record_id!r} must be linked from at least one "
                f"requirement whose requiredPositiveProofKinds includes a {label} "
                f"proof kind ({allowed})"
            )
    return errors
