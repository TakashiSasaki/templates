...
if ARGV.length > 1
  warn "usage: ruby #{File.basename(__FILE__)} [SKILL_ROOT]"
  exit 2
end

repository_root = File.expand_path(ARGV.first || Dir.pwd)
unless File.directory?(repository_root)
  warn "Skill root is not a directory: #{repository_root}"
  exit 2
end
