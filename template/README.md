...
The template includes validators for a concrete Skill. From the Skill root, run:

```sh
ruby .github/scripts/validate-skill-repository.rb
```

The validation entry point checks frontmatter, the machine-readable profile selection, exact operational-resource declarations, retained contract requirements, unresolved placeholders, public-interface consistency, runtime/interface alignment, Git link boundaries, and concrete-Skill completion rules.
...
The included GitHub Actions workflow runs the same validation. Retain, replace, or remove that workflow according to the concrete repository’s CI policy; it is not an operational Skill resource.
