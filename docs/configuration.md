...
A repository-bound review first establishes an **active trusted repository-policy root**. By default this is the exact current pull-request base tip captured at review start; a caller may instead supply another immutable repository-policy revision only when repository policy explicitly authorizes that root. Proposed-head configuration, lock state, policy, generated outputs, adapters, or Skill copies are review data and do not become authority for the same review.
...
Unless the caller supplies an explicitly authorized immutable out-of-band procedure/toolchain revision, the active trusted configuration must include `pr-review` in `skills.enabled`, and the procedure revision is derived from the validated lock's toolchain revision. A toolchain containing `pr-review` does not by itself authorize that Skill. An explicitly authorized out-of-band procedure revision is the only path that may bypass repository Skill enablement; it remains independent of proposed-head content and is recorded in review evidence.
...
Checked-in generated review projections are not trusted from path/renderer metadata alone. Before use, the active trusted lock's input and output digests must match the trusted snapshot, and deterministic check/regeneration with that exact immutable toolchain must reproduce the bound semantic and adapter projections byte for byte. A stale, manually altered, unverifiable, or non-reproducible projection fails closed; the presence of a lock digest is not by itself proof that arbitrary generated bytes implement the canonical inputs.
