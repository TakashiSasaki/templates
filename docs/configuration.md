...
The current automated-review trust contract has exactly one repository-facing bootstrap path. An installed immutable `agent-policy` Skill performs trusted bootstrap before `pr-review` executes. The bootstrap records stable repository identity, pull-request identity, the exact current base revision, and a repository snapshot proven to represent that same repository at that exact base. The exact base snapshot is the only active repository-policy root for the review.
...
Missing, duplicate, disabled, role-swapped, unsupported, unsafe, or otherwise inconsistent bindings fail closed instead of falling back to naming conventions. Configuration, lock, and projection paths must remain repository-relative and root-confined, must not enter `.git` or another reserved namespace, and must not contain a symlink component.
...
Immediately before serialization, the procedure re-resolves stable repository identity, base, head, and the complete best-common-ancestor set. Repository identity must remain equal to bootstrap evidence; a change fails closed. If the base changes within the same repository, the current run returns to the installed immutable bootstrap and re-establishes configuration, lock, `skills.enabled`, generated Skill provenance, output bindings, and generated projection bytes from the replacement exact base. If procedure revision or Skill bytes change, the old Skill cannot continue and review restarts under the newly verified Skill. Head/merge-base-only movement may be refreshed within the same verified procedure while base/bootstrap authority stays stable.
...
Codex, Gemini, Antigravity, or another engine may consume these generated documents; engine invocation details remain outside semantic policy. The installed immutable `agent-policy` Skill owns only trusted procedure bootstrap, while the verified generated `pr-review` Skill is the sole review-execution procedural authority. A retained canonical prompt is only a thin non-normative invocation template that supplies repository/PR and output-binding data; it does not select bootstrap authority or a procedure revision. Other provider-specific event names, APIs, or serialization contracts require their own adapter-only renderer or external adapter rather than changes to review-rule modules.
