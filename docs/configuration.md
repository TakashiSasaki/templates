...
The current automated-review trust contract has exactly one repository-facing bootstrap path, but the existence of an installed `agent-policy` Skill is not itself sufficient authority. Before that Skill may perform trusted bootstrap, the deployment/hosting dispatcher independently authenticates its installed Skill-source tree against a deployment-managed installation attestation outside both the installed Skill tree and the repository under review.

The attestation binds the independently trusted full-SHA remote-installer revision, the immutable Skill-source revision selected by that installer, the exact installed Skill root, and a **closed path/type inventory of the complete installed Skill tree**, with SHA-256 for every regular file. Verification requires exact inventory equality: added or missing files/directories, file↔directory substitutions, symbolic/hard links, oversized content, or changed file bytes fail closed. The exact-SHA remote installer supports both attestation creation and later `--verify-only` verification. The dispatcher must authenticate the installer script itself from its independently pinned revision before accepting that verification result. `runtime-manifest.json` remains a separate runtime-selection contract and does not authenticate the bootstrap algorithm stored in the installed Skill tree.

The authenticated installed Skill tree remains immutable trust material during the review invocation. Trusted bootstrap must not create Python bytecode caches, logs, temporary files, or state inside that tree. The installed Python entry points disable bytecode writes before importing sibling Skill modules, and the dispatcher runs trusted bootstrap with bytecode writes disabled as well (for example `python -B`) or from an equivalently isolated read-only verified tree. Runtime/cache state remains outside the attested Skill root. If normal execution can change the attested inventory, trusted bootstrap is unavailable and fails closed.
...
After successful reproduction, `pr-review` binds the exact verified semantic and adapter bytes to the review run. It either retains immutable in-memory/content-addressed copies outside the writable repository snapshot or records their cryptographic digests and reverifies the source bytes immediately before each later consumption. The procedure never silently reopens mutable projection paths after verification. Final serialization uses the exact adapter identity bound to the semantic analysis; projection-byte drift before serialization fails closed, and semantic-projection drift invalidates the prior semantic analysis.
...
At review start, `pr-review` requires both repository identity and pull-request identity to match bootstrap evidence. Immediately before serialization, it re-resolves pull-request identity, repository identity, base, head, the complete best-common-ancestor set, and the semantic/adapter projection identities used by the completed analysis. Pull-request or repository identity drift fails closed even if commits happen to match.

If the base changes within the same PR/repository, the current run returns to the already deployment-authenticated installed bootstrap and re-establishes configuration, lock, `skills.enabled`, generated Skill provenance, output bindings, and generated projection bytes from the replacement exact base. **All evidence classifications, semantic analysis, limitations, and the candidate serialized result produced under the old trusted root are discarded.** The replacement base's projections are rebound and the complete review procedure runs again even if the `pr-review` procedure revision and Skill bytes are identical. If procedure bytes change, the old Skill cannot continue and the new run starts under the newly verified Skill. Head/merge-base-only movement may refresh affected analysis only while PR/repository/base/bootstrap and run-bound projection identities remain stable.

This explicit invocation binding avoids a schema transition while preserving deterministic context, renderer-role, procedure, generated-byte, and serialization selection. Introducing a machine-declared Skill-to-output binding into `.agent-policy.yml` would change the configuration trust contract and requires a separate architecture decision.
