...
| `ci` | `candidate commit` | Policy CI is reproducible on the Ubuntu runner's Python, neutralizes external Python and pip inputs, verifies the installed distribution set, and exercises the runtime-distribution checks when runtime behavior changes. | `.github/workflows/ci.yml`, `.github/workflows/runtime-distribution.yml`, `tests/test_ci_reproducibility.py`, `tests/test_runtime_distribution.py` |
...
both checks. The default Ubuntu runner Python is the local readiness target.
Release/deployment qualification and GitHub artifact checks also remain remote.
