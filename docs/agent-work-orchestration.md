...
## Construction ahead of qualification

Keep speculative implementation parallel; delay knowingly disposable revision-bound evidence. In `A -> B -> C`, track construction and qualification frontiers separately using the [existing identity lifecycle](revision-bound-qualification.md#two-frontiers-in-a-stack). A pending review alone does not stall B/C. A known A mutation that will invalidate descendant exact-head evidence defers that intentional final cycle while safe implementation and focused diagnostics continue.

The Work ledger can record expected prerequisite invalidators, affected evidence, the deferral reason and boundary that releases it in its existing Stability/Evidence/Dependencies groups. Record obsolete-run supersession and still-applicable evidence separately. These observations do not establish acceptance, introduce a serialization schema, or replace the review-finding ledger. Before final qualification and the authorized review, use the [bounded execution-path check](staged-ci.md#qualification-sequencing-and-effective-coverage) to establish that claimed assertions run in the authoritative lane.
