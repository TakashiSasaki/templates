...
Progression determines construction ordering. Completion determines where the agent stops. Therefore serial-pr does not intrinsically mean "review and merge before task completion": with agent-review-and-merge, one validated member proceeds through review, guarded merge, then the next member; with human-handoff, the current validated member stops at HANDOFF_READY without initiating a new merge-acceptance review request or merge by default. An explicit task may instead authorize one final diagnostic whole-stack architecture/dependency/completeness audit after the candidate is stable enough for handoff. That audit does not authorize merge, does not replace later exact-head acceptance review, and must not create a review-retry loop.

Stacked progression likewise does not prescribe cumulative review. Under agent-review-and-merge, each stacked member may use its own completed independent exact-head review, or multiple members may be covered by one explicit cumulative review when the canonical coverage bindings are satisfied. A tip-only approval is not lower-member cumulative coverage. Under human-handoff, the validated whole stack remains open and unmerged without initiating a new merge-acceptance review request by default. When an explicit task authorizes the final whole-stack architecture/dependency/completeness audit, the audit remains diagnostic rather than ordinary per-member merge-acceptance evidence, does not authorize merge, and must not create a review-retry loop.
