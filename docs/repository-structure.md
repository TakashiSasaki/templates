...
ファイル名を選択すると、同じビルドで書き出された内容をダイアログで確認できます。UTF-8テキストとPNG・JPEG・GIF・WebP画像をプレビューし、それ以外のバイナリまたは512 KiBを超えるファイルはGitHub上の表示へ案内します。
...
| `tests/` | 設定検証、初期化、レンダリング、ロック整合性、パス安全性、リポジトリ保守判定、プレビュー資産分類を検査します。 |
