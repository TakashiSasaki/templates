...
- an empty `PYTHONPATH` for environment creation, installation, validation, and tests;
...
CI clears `PYTHONPATH`, clears and recreates `.venv`, installs only the entries enumerated in `requirements-dev.lock` with dependency resolution disabled, then uses that virtual environment for `pip check`, both public validator entry points, and the complete unit-test suite. Clearing is required because `venv` otherwise reuses an existing environment directory and can retain distributions installed by an earlier lock. Clearing `PYTHONPATH` is required because CPython prepends those external directories to its import path even after virtual-environment activation; packages from another environment could otherwise affect imports or distribution discovery and conceal a lock omission. Isolation from the setup interpreter is also required because `pip check` validates the currently visible installed graph. Disabling dependency resolution prevents an omitted transitive or conditional dependency from being silently retrieved from the package index.
...
Clear and recreate an isolated environment, remove any externally supplied Python import path, then install exactly the locked graph:
...
4. clears `PYTHONPATH`, clears and recreates the virtual environment without system site packages, installs the lock with dependency resolution disabled, and runs `pip check` there;
