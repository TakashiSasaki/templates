...
- empty `PYTHONPATH`, `PYTHONSAFEPATH`, `PYTHONPLATLIBDIR`, `PYTHONMALLOC`, `PYTHONIOENCODING`, `PYTHONTRACEMALLOC`, and `PYTHONINSPECT` before the first Python invocation and throughout validation;
...
- the complete arbitrary exact dependency graph in `requirements-dev.lock`; and
