...
4. clears `PYTHONPATH`, `PYTHONSAFEPATH`, and `PYTHONPLATLIBDIR` before the first Python invocation, removes requirement, constraint, editable, dependency-group, script-metadata, progress-bar, and transport, certificate, timeout, default-timeout, and retry pip inputs, disables pip configuration files, and clears and recreates the virtual environment without system site packages;
