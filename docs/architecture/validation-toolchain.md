...
- empty `PYTHONPATH` and `PYTHONSAFEPATH` before the first Python invocation and throughout validation;
...
4. clears `PYTHONPATH` and `PYTHONSAFEPATH` before the first Python invocation, removes requirement, constraint, editable, dependency-group, script-metadata, and transport, certificate, timeout, and retry pip inputs, disables pip configuration files, and clears and recreates the virtual environment without system site packages;
