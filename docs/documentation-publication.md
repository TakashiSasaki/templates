...
The workflow does not check out Composition or any retired provider branch. Its only cross-authority checkout is the single Integration protocol file at the reviewed full SHA above; it does not execute a mutable `integration` branch tip or consume Integration reader IA as Policy semantics. The repository default branch is irrelevant to the Policy event boundary. The workflow is scoped explicitly to pushes on `policy` and pull requests targeting `policy` or authority-local `policy-*` bases, and deliberately omits `workflow_dispatch` and `workflow_call`.

All third-party actions are pinned to full commit SHAs. `requirements-docs.txt` records the reviewed direct documentation dependencies with arbitrary exact equality (`===`), while `requirements-docs.lock` records the complete arbitrary-exact graph used by the locked documentation environment. `scripts/verify_docs_environment.py` rejects missing, extra, or version-mismatched distributions outside that lock, excluding only the virtual environment's bootstrap `pip`. Dependency updates require a reviewed re-resolution and successful strict build.
...
Remove external Python and pip inputs before the first Python invocation, then reproduce the enabled documentation build from the reviewed lock. Make a separate checkout of Integration commit `a30699cf7dc56bf3ef7a1b6fd8f6ffd45cdd426d` available before the protocol step:
...
. .venv/bin/activate
python3 -m pip install --isolated --disable-pip-version-check --no-deps --requirement requirements-docs.lock
...
The Integration checkout used for `INTEGRATION_PUBLICATION_PROTOCOL_ROOT` must identify exactly the reviewed full SHA above. A mutable branch checkout is not equivalent evidence. The local metadata command uses the checked-out Policy commit and records `run_id` and `run_number` as `0`, distinguishing a local build from a GitHub Actions run while preserving the same `docs/build-info.json` shape and footer behavior. The workflow calls the same generator with the actual repository and run identifiers.
...
The Integration publication protocol checkout must remain pinned to a reviewed full Integration commit SHA. Updating that pin is a cross-authority protocol-consumption change and requires review of the exact Integration revision; it must not silently follow `integration`, another branch, a tag, or a pull-request merge ref.
