...
- pushes to `policy` and manual runs dispatched from `policy` perform the same build, upload the `site/` directory as a GitHub Pages artifact, and deploy it through the `github-pages` environment.
...
5. Run **Policy documentation** manually from the `policy` branch if the merge-triggered deployment ran before the settings cutover.
