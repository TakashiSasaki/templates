...
The workflow does not fetch the unrelated `skill`, `site`, or `webapp` branch histories. The repository default branch is irrelevant to this boundary. The workflow is scoped explicitly to pushes and pull requests for `policy` and deliberately omits `workflow_dispatch` and `workflow_call`.
