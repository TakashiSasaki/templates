...
The independent `Policy runtime distribution` workflow retains its own applicability classifier and runs the canonical runtime plus skill-source checks in parallel with normal Policy CI. It uses the Ubuntu runner's Python directly; the runtime lock and the isolated environment created by the semantic smoke test remain part of the contract. This separation demonstrates that staged CI is a dependency and evidence model, not a requirement to place every repository check in one linear workflow.
