...
<section class="portal-hero" aria-labelledby="portal-overview-title">
  <div class="portal-hero__copy">
    ...
    <p class="portal-hero__lead">
      Understand how Skill contracts, shared agent Policy and its toolchain, and
      Web application templates and contracts remain independently owned while
      appearing through one reproducible documentation portal.
    </p>
  ...
  <div class="portal-hero__visual">
    <img src="../images/landing-architecture.svg" alt="Skill, policy, and Web application development systems connected through a validated documentation portal">
  </div>
</section>
...
<section class="portal-section" id="choose-a-template" aria-labelledby="choose-a-template-title">
  <div class="portal-section__heading">
    <p class="portal-kicker">Choose a development system</p>
    <h2 id="choose-a-template-title">Three responsibilities, one engineering repository</h2>
    <p>Start from what you need to build or govern. Each branch family owns a distinct responsibility boundary.</p>
  </div>
  ...
  <div class="portal-card-grid">
    ...
    <a class="portal-card portal-card--webapp" href="../webapp/">
      ...
      <span>Application templates and contracts, implementation and release evidence, validation, responsibility boundaries, and migrations.</span>
...
<section class="portal-section portal-publication" aria-labelledby="publication-model-title">
  <div class="portal-section__heading">
    ...
    <h2 id="publication-model-title">Reviewed source branches become one reader-oriented portal</h2>
    <p>In Site publication terminology, a <strong>Provider branch</strong> is one of the repository-internal <code>skill</code>, <code>policy</code>, or <code>webapp</code> source branches that supplies canonical content to the integrated publication. It is not an external cloud, LLM/API, or identity provider. The <code>site</code> branch selects reviewed commits from those Provider branches without merging their unrelated histories.</p>
  </div>
  <div class="portal-publication__layout">
    ...
    <div class="portal-principles">
      <article>
        <span>01</span>
        <div><h3>Explicit boundary</h3><p>Each Provider branch owns a publication catalog that explicitly allowlists its public inputs. Uncataloged files do not become published documentation by proximity.</p></div>
      </article>
      <article>
        <span>02</span>
        <div><h3>Immutable inputs</h3><p>Provider branch revisions are recorded as full commit SHAs rather than mutable branch tips.</p></div>
      </article>
      <article>
        <span>03</span>
        <div><h3>Validated output</h3><p>The integrated build checks navigation, links, fragments, repository trees, terminology, and provenance.</p></div>
      </article>
    </div>
  </div>
</section>
...
The `skill`, `policy`, and `webapp` source branches remain independent and retain
ownership of their canonical documents. In the publication architecture these
three source branches are called **Provider branches**. The term describes their
repository-internal role as content sources; it does not refer to an external
service provider. The `site` branch is the integration and publication branch,
not a Provider branch. It selects reviewed Provider branch commits, validates the
...
Provider branch inputs are recorded as lowercase full 40-character commit SHAs
in `publication-sources.json`. A `site` commit therefore identifies exact
`skill`, `policy`, and `webapp` revisions rather than mutable branch tips.
...
documentation. The site build checks out each Provider branch independently at
its locked commit, assembles one temporary project, performs a strict static-site
