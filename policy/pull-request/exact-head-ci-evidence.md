...
- **Base-authoritative**: The classifier logic and control definitions must resolve from the authoritative base revision (or an immutable authority) rather than proposed code, so proposed changes cannot alter their own classification rules.
