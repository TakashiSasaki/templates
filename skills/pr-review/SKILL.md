...
11. Immediately before serialization, enter a stability loop. Re-resolve the **pull-request identity**, stable repository identity, pull-request base tip, proposed head, and complete set of best common ancestors from the hosting/repository system. Require pull-request identity and repository identity to remain the identities bound by bootstrap evidence and require the ancestor set to still contain exactly one revision. If pull-request identity, repository identity, base tip, head, and unique merge-base equal the identities used by the current analysis, and bootstrap authority remains current, exit the loop and proceed directly to serialization. If pull-request identity or repository identity changes, fail closed. If the histories become unrelated or have multiple best merge bases, fail closed.
