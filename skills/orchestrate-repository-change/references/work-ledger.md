...
- **Head movement:** exact-head CI/review evidence for the old SHA cannot qualify the new SHA. Preserve its historical locator, mark its qualification binding stale, and acquire new exact-head evidence when the selected boundary requires it. Semantic implementation or repair progress can remain complete while qualification becomes pending. However, under the canonical qualification evidence reuse policy, merge progression does not itself invalidate qualification evidence: when candidate tree and required bindings remain unchanged across stacked landing or history-only evolution, evaluate applicability (`applicable`, `stale`, `unknown`), record the reuse decision, and reacquire only invalidated qualification.
