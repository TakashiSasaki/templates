...
6. **Keep provisional work productive.** Naturally triggered CI on a provisional candidate does not freeze it. While independent validation is pending, continue dependency-safe stacked work and other productive parallel work that does not invalidate the candidate or propagate a known defect.
7. **Freeze at the authority boundary.** When merge acceptance, final whole-stack audit, release, publication, provenance, or another revision-bound operation requires a qualification head, stabilize prerequisites and freeze the intended candidate.
8. **Complete exact-head qualification.** At the frozen boundary, require every applicable exact-head check currently owned by repository authority, including full qualification when that boundary requires it. Earlier-stage success cannot substitute for missing later-stage evidence.
9. **Supersede stale expensive work.** When a newer candidate makes an in-flight expensive result unable to satisfy any current evidence requirement, cancel or supersede it when the provider safely supports that action. Preserve evidence whose bindings remain valid and invalidate only the affected evidence.
