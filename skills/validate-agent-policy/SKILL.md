...
3. If a suitable `agent-policy` command is already available, run:
   - `agent-policy --repository . validate --config {{ config_path }}`
   - `agent-policy --repository . check --config {{ config_path }}`
...
6. Do not edit generated files directly. Modify semantic inputs, run the pinned toolchain's `render` command with `--config {{ config_path }}` when synchronization was requested, and then repeat `validate` and `check`.
...
For GitHub Actions, use the pinned composite action from `toolchain.repository` at `toolchain.revision` with `command: check` and `config: {{ config_path }}`. The upstream reference workflow is `templates/workflows/check-agent-policy.yml.j2` in the toolchain repository.
