...
Under human-handoff, do not initiate a new review request by default for merge acceptance. An explicit task may authorize one final diagnostic whole-stack architecture/dependency/completeness audit after the stack is stable enough for handoff. That audit is not ordinary per-member merge-acceptance evidence, does not authorize merge, does not waive later exact-head review requirements, and must not create a review-retry loop. Existing reviews may be inspected or reported, but the handoff path itself does not acquire acceptance review. Human handoff remains a completion boundary and does not turn missing review evidence into acceptance evidence.
