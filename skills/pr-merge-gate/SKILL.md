...
Review acquisition and completion mode are separate concerns. The agent-review-and-merge path may proceed through the normal evidence and guarded-merge states. The human-handoff path stops at HANDOFF_READY after authorized implementation and validation work; it does not initiate a new merge-acceptance request by default through reviewer assignment, provider invocation, requested-reviewer state, or any other review-request mechanism, does not close a pull request, and does not authorize or execute a merge or imply REVIEW_COMPLETE, MERGE_READY, or MERGED. Under human-handoff, do not initiate a new review request by default. Human handoff does not initiate a new review request merely because a PR is open or a reviewer is available. When an explicit task requires a diagnostic request, use the canonical adaptive scope-selection rule and issue the one logical request required by the current final packet after the stack is stable. This is a diagnostic whole-stack or bounded-scope request as selected by the current bindings; it is not ordinary per-member merge-acceptance evidence, does not authorize merge, does not waive future exact-head review requirements, and must not create a retry loop. Additional review is permitted later when new evidence, changed bindings, or uncovered scope justifies it; an equivalent in-flight request must be reconciled rather than duplicated. Existing reviews may be observed, inspected, and reported. Human handoff is not a waiver of the independent-review or merge requirements that apply to a later continuation. It is not merge-ready.
...
- `pull-request.select-review-scope-from-current-bindings` — `policy/pull-request/review-scope-selection.md`
...
For GitHub-facing evidence, use `references/github-review-result-discovery.md` to identify the review purpose and latest applicable review request or request-less cycle anchor when present, then inspect the applicable submitted reviews, ordinary pull-request comments, inline review comments, resolvable review threads, reactions with defined semantics, and provider completion/failure/limitation signals. Reconstruct the logical result before deciding whether review is pending, complete, problem-free, or contains findings.

First distinguish confirmed absence from ambiguous evidence. When no applicable review request exists, no completed request-less result is discovered, and no other applicable review-result evidence is present, classify review evidence as absent and enter `REVIEW_EVIDENCE_PENDING` or `BLOCKED_REVIEW_MISSING`; confirmed absence is not `BLOCKED_REVIEW_APPLICABILITY_UNKNOWN`. If applicable review-result evidence is discovered but its cycle, purpose, or reviewed revision cannot be established, enter `BLOCKED_REVIEW_APPLICABILITY_UNKNOWN`; do not infer completion or `no findings` from a clean or incomplete surface.

Before intentionally initiating revision-bound review acquisition, apply `pull-request.disposition-known-findings-before-review-reacquisition` and evaluate the complete logical finding backlog with `references/review-finding-ledger.md`. Apply the same complete known-finding reacquisition gate at this decision boundary. Every known material actionable finding applicable to the candidate must already have either a repair validated for the current proposed head or an evidence-backed no-change disposition validated against the current proposed head and applicable authority, and the required finding-level closure evidence must be recorded on an auditable review or pull-request surface. If any known material item lacks that validated outcome or required closure evidence, enter `BLOCKED_REVIEW_FINDINGS` and do not invoke the reviewer. Only after this known-finding gate is satisfied, apply `pull-request.preflight-review-acquisition` and the revision-binding preflight below.
...
When review acquisition is GitHub-facing and the mechanism permits output guidance, apply `references/github-review-finding-representation.md` when constructing that request. Request remediation-friendly representation in substance: one separate resolvable inline thread for each independently actionable finding with an honest causal changed-line anchor when GitHub supports it, no bundling of unrelated findings, no fabricated inline anchors, and separately identifiable unanchorable findings. This is provider-specific transport guidance only; it does not define semantic finding validity, severity, review completion, or a required provider result schema.

Under human-handoff, do not initiate a new merge-acceptance review request by default. Before an explicitly authorized diagnostic request from an explicit task, apply the adaptive scope-selection rule, the complete known-finding reacquisition gate, and the revision-binding preflight. The request is a whole-stack or bounded-scope diagnostic selected from current evidence; a whole-stack selection is an architecture/dependency/completeness audit, not automatic per-member acceptance evidence. It is not ordinary per-member merge-acceptance evidence, does not authorize merge, does not waive later exact-head review requirements, and must not create a review-retry loop. If the same purpose, candidate, scope, contract, and input binding is already in progress or has unknown submission status, reconcile that request instead of sending another one. Existing reviews may be inspected or reported, but human handoff remains a completion boundary and does not turn missing review evidence into acceptance evidence.
