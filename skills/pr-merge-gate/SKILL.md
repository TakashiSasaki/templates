...
Under human-handoff, do not initiate a new merge-acceptance review request by default. Before an explicitly authorized diagnostic request, apply the adaptive scope-selection rule, the complete known-finding reacquisition gate, and the revision-binding preflight. The request is not ordinary per-member merge-acceptance evidence, does not authorize merge, does not waive later exact-head review requirements, and must not create a retry loop. If the same purpose, candidate, scope, contract, and input binding is already in progress or has unknown submission status, reconcile that request instead of sending another one. Existing reviews may be inspected or reported, but human handoff remains a completion boundary and does not turn missing review evidence into acceptance evidence.
