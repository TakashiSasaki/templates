...
This reference is provider-neutral procedure support for `pr-merge-gate`. It does not create a new acceptance gate, freshness interval, waiting period, or semantic pull-request rule. It controls when already-justified head-changing repairs are applied so exact-head evidence is not repeatedly invalidated by avoidable mutation churn. The canonical `pull-request.defer-revision-bound-qualification-until-required` rule decides when a provisional construction state should remain provisional instead of being deliberately promoted into revision-bound qualification.

## Stabilize before intentionally acquiring expensive acceptance evidence

Before intentionally requesting independent review or starting another expensive revision-bound acceptance evaluation, establish as far as reasonably possible that the authorized implementation is complete, known self-audit findings are dispositioned, the logical finding backlog in `review-finding-ledger.md` has no known material actionable item awaiting a validated repair, evidence-backed no-change disposition, or required closure evidence, focused validation is complete, required generated projections are internally coherent, known compatible head-changing repairs are batched, no known material defect remains unresolved, and no known downstream immutable-identity materialization is waiting on an upstream semantic repair or other still-mutable prerequisite.

This is a candidate-stabilization discipline, not a new acceptance gate. It has no fixed duration, does not require perfection, does not prevent PR creation, and does not require waiting for earlier CI or review before dependency-safe implementation continues. Naturally triggered CI may run before stabilization. A successful run on a provisional construction head remains useful observed evidence but does not by itself convert that head into a final qualification identity. Do not deliberately request independent review for a candidate already known to be intermediate or deliberately materialize a downstream immutable identity that is still expected to follow mutable prerequisites.

For a final whole-stack review, treat required CI and review as ordered revision-bound evidence rather than parallel latency-hiding work. Freeze the intended qualification heads first, then require all applicable required CI on those exact heads to have completed successfully before requesting the final review. Pending required CI means the review candidate is not yet qualified for that request. Failed or cancelled required CI requires repair or explicit disposition and requalification. If a head changes after revision-bound review or qualification has been acquired, reacquire the required evidence for the affected exact head before relying on the replacement candidate at the same boundary. This ordering does not block construction of later stack members and is not a time-based waiting gate.
...
A construction head may therefore be exact and testable while the candidate remains provisional. Preserve semantic repair progress and diagnostic evidence whose actual binding remains applicable without prematurely declaring the head a qualification head. Use qualification validation when the current candidate is intentionally frozen to support revision-bound acceptance, final review, release, publication, merge readiness, or another authority-defined completion boundary. Qualification must include every check required by the applicable repository authority and must bind those results to the exact candidate revision or artifact.
...
Once a candidate head has entered CI or independent review for an intended revision-bound boundary, treat that SHA as stable unless a justified repair, scope correction, conflict resolution, or other necessary head-changing action is ready to apply.
...
6. invalidate and reacquire only the exact-head evidence whose actual binding changed when that evidence is required for the next revision-bound boundary.
...
Do not intentionally reacquire merge-acceptance review between compatible repairs that are already known and verified when that would create avoidable one-finding-at-a-time review/head churn. Once the coherent replacement candidate is ready and the next applicable revision-bound boundary requires qualification, freeze that candidate and apply the canonical review-reacquisition rule to the complete known backlog rather than the last edited thread alone. Request or collect new exact-head CI/review evidence only after the coherent replacement candidate is ready for the applicable qualification boundary.
...
A mutation batch creates one new construction head. Mark evidence stale according to its actual bindings:

- exact-head CI, exact-head review, and head-bound scope evidence for the former SHA are stale when those artifacts are being relied upon as exact-revision evidence;
- target-branch evidence is affected only when its own binding changed;
- finding identity, source review, semantic repair state, and other semantic ledger state survive a head change unless the change invalidates their applicability;
- evidence independent of the changed head remains reusable when current policy establishes continued validity.

A changed construction head does not by itself require immediate deliberate reacquisition of every final qualification artifact when the candidate remains provisional and no current boundary requires those artifacts. Naturally triggered checks may still run and be observed. Before the replacement candidate is reported as qualified, reviewed, merge-ready, released, published, or otherwise complete at a revision-bound boundary, reacquire every required exact-head gate whose binding changed.

Do not deliberately produce a sequence of partial candidate SHAs that each trigger the same expensive evidence cycle when the remaining known repairs could have been applied together.

## Relationship to review feedback disposition and the ledger

`review-feedback-disposition.md` decides what a verified review item means and what remediation it requires. `review-finding-ledger.md` keeps the entire known backlog and its closure state recoverable across provider surfaces and head changes. This reference decides when compatible, already-justified head-changing remediations should be applied and when their replacement construction head should remain provisional until qualification is actually required.
...
Mutation batching is an efficiency discipline, not acceptance evidence. Candidate stabilization and delayed qualification are likewise efficiency disciplines, not acceptance evidence. At an applicable revision-bound completion boundary, reacquire every exact-head gate invalidated by the mutation before reporting the replacement candidate as successful, and require the normal merge-gate workflow to reach its required state.
