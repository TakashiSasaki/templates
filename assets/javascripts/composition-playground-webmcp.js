(function (scope) {
  ...
  function validateIntent(raw, context) {
    if (!raw || raw.schema_version !== 1 || raw.projection_id !== INTENT_ID || raw.resolution_projection_id !== context.projection.projectionId || raw.strategy !== STRATEGY) throw new Error("unsupported WebMCP intent projection");
    if (!raw.source || raw.source.repository !== "TakashiSasaki/templates" || raw.source.authority !== "composition" || !exactSha(raw.source.revision)) throw new Error("invalid WebMCP intent projection provenance");
    if (raw.source.revision !== context.projection.semanticRevision) throw new Error("WebMCP intent and resolution projections have different semantic revisions");
    if (!raw.encoding || raw.encoding.nonnegative !== "canonical outcome id" || raw.encoding.negative !== "-(error index + 1)" || !Array.isArray(raw.errors) || !Array.isArray(raw.recipes)) throw new Error("invalid WebMCP intent projection encoding");
    return raw;
  }
  ...
  async function loadIntent(root, context, generation) {
    ...
    if (!core || typeof core.decodeProjectionResponse !== "function") throw new Error("canonical projection decoder is unavailable");
    const raw = await core.decodeProjectionResponse(response, url);
    if (generation !== state.generation || root !== state.root) return null;
    return validateIntent(raw, context);
  }

  function recipeIntent(intent, recipeId) { return intent.recipes.find((entry) => entry && entry.id === recipeId) || null; }
  ...
  function setModeControls(root, mode, available) {
    root.querySelectorAll('[name="playground-webmcp-intent"]').forEach((radio) => { radio.disabled = !available; radio.checked = available && radio.value === mode; });
  }
  function coreCheckbox(root) { return Array.from(root.querySelectorAll('[data-playground-optionals] input[type="checkbox"]')).find((node) => node.value === COMPONENT) || null; }
  function renderList(document, node, values) {
    while (node.firstChild) node.removeChild(node.firstChild);
    for (const value of values) { const li = document.createElement("li"); li.textContent = value; node.appendChild(li); }
  }
  function render(context) {
    ...
    } catch (error) { fail(root, error instanceof Error ? error.message : String(error)); panel.hidden = true; }
  }
  ...
  function mount(context) {
    ...
    if (state.root !== root) {
      ...
      void loadIntent(root, context, generation).then((intent) => { if (!intent) return; state.intent = intent; render(state.context); }).catch((error) => fail(root, error instanceof Error ? error.message : String(error)));
    } else { state.context = context; render(context); }
  }
