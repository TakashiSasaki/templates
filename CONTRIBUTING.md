...
Pull requests to `skill` that change canonical Markdown, `docs/`, or `assets/` are assembled and built against the current `site` branch by **Check documentation site compatibility**. The pull-request run uses GitHub's proposed merge commit, records both exact source commits in the generated artifact, and never deploys it.

The same non-deploying check can be started manually to detect documentation drift. GitHub schedules workflows only from the repository default branch, which is `site`, so this `skill`-branch workflow does not claim a weekly scheduled run. No workflow on `skill` deploys GitHub Pages; publication is outside this branch's authority.
