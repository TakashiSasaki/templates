...
The explicit-root form must resolve every contract and operational resource relative to the supplied skill root rather than the caller's current working directory. If the target is inside a Git worktree, the validator inspects its real index for forbidden operational-resource gitlinks. If a flattened archive target has no Git metadata, the supported orchestrator supplies a temporary empty index only for that metadata-only check; every filesystem, declaration, symlink, profile, and contract rule still executes against the target, and validation must not write into it.
...
The installation-mode smoke under `.github/scripts/test-installation-modes.rb` must materialize one concrete `script-assisted` skill from the canonical root, commit it once, and install that exact commit by direct clone, project submodule, and prefixed Git archive extraction. The archive must be flattened so `SKILL.md` is directly under the final `.agents/skills/<name>/` directory. All three installations must have identical non-Git inventories, file bytes, and file modes; pass complete validation from an unrelated working directory; and execute the same deterministic helper without input mutation, network use, or dependency installation. The smoke must verify real clone and submodule metadata, verify that archive validation requires no repository metadata in the target, reject an unflattened archive wrapper, and reject undeclared operational files in every mode. It does not publish releases, fetch remote archives, select profiles or licenses, or create a generator interface.
