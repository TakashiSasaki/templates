...
The mobile layout command requires a local Chrome executable and writes screenshots plus `metrics.json` under `build/mobile-visual`. Use workflow-call revision overrides only for deliberate compatibility testing. Normal builds use the reviewed full-SHA lock file. Repository-tree links, preview URLs, repository-browser snapshots, and guided-navigation graph/viewer output always use the actual checked-out commits.