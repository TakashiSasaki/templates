...
The installable shell and runtime freshness behavior are owned entirely by the `site` branch. `FRESHNESS.md` is the normative document-cache, Glossary-model cache, and freshness-state contract.
...
- `assets/service-worker.js` keeps the versioned static shell cache separate from both `templates-portal-documents-v1` and `templates-portal-glossary-v1`. The shell precaches the manifest, icon, Site-owned common stylesheets, and Site-owned local JavaScript needed to render previously viewed documentation. Generated documents and `/glossary/index.json` are never added to that static shell namespace.
...
A document network request has a 1500 ms soft timeout. The timeout never aborts the original request. If the network is still pending and no safe stored document exists, the browser keeps waiting for that original request. If a safe stored document exists, it may be shown with the explicit `checking` state while the original network request continues. Background completion compares the stored and network `templates-site-revision` identity and converges to `verified-current`, `update-available`, or `cached-unverified` without replacing the visible DOM automatically. `update-available` exposes an explicit Reload action. The live capability contract exposes `softTimeoutMs: 1500` so browser checks and future clients can verify the active timing contract rather than duplicating an implicit constant.

For full browser navigation, cached fallback HTML carries its own fixed freshness notice and therefore does not require a pre-existing client acknowledgement. For instant navigation, `pwa.js` applies the persistent `checking` or `cached-unverified` status element and acknowledges it through a generation- and worker-instance-bound `MessageChannel` before the Service Worker may expose cached HTML. This makes stale indication a safety condition: an old/unaware controlled client that cannot acknowledge the UI receives no unindicated cached document. Document and freshness messages carry the exact URL, request generation, and worker-instance epoch. `pwa.js` rejects late updates for a previous document and retires older convergence when a newer network representation actually commits. An uncorrelated Zensical `document$` event alone never clears a freshness warning.

The integrated `/glossary/index.json` model uses the independent `templates-portal-glossary-v1` runtime cache and remains network-first with HTTP-cache revalidation. The 1500 ms document soft timeout does not implicitly apply to this Glossary route. Same-origin HTTP 200 JSON may refresh that cache. HTTP 404/410 is authoritative and removes the saved model; generation ordering prevents delayed older writes or deletes from superseding a newer result. HTTP 5xx and network failures may expose the saved model only when the requesting Glossary runtime explicitly opts in to cached-unverified semantics, and the inline dialog must display the unverified status before presenting that definition. A cached-unverified model is not retained indefinitely by the page runtime: the next Glossary activation retries network-first verification so connectivity recovery can converge without a page reload.

Do not precache the portal home, generated documentation pages, or the integrated Glossary model in the static shell. Routine shell-asset content changes converge through HTTP revalidation. Increment `CACHE_NAME` when the shell cache strategy, namespace, or compatibility contract changes so activation can delete incompatible shell caches. Preserve `templates-portal-documents-v1` and `templates-portal-glossary-v1` across compatible shell updates. Change either runtime-cache namespace only when its own storage or representation contract becomes incompatible; activation may delete older `templates-portal-glossary-*` namespaces without deleting the compatible document cache.

`tests/test_pwa_assets.py` owns the source-level PWA contract, including manifest shape, shell-asset coverage, shared SVG safety, registration wiring, generated metadata insertion and preview exclusion, duplicate/conflicting metadata rejection, document classification, runtime cache separation, response cloning, authoritative deletion, fail-closed stale indication, and cache-version transition. `tests/test_glossary_offline_cache_contract.py` owns the source-level Glossary network-first/cache-fallback contract, including opt-in, generation ordering, cached-response decoration, retry after cached-unverified use, and cache namespace cleanup. `tests/test_pwa_slow_convergence.py` owns the source-level slow-timeout/state-ordering regression contract. `scripts/check_pwa_freshness.py` owns the Chromium lifecycle contract for HTTP-cache revalidation, runtime document-cache update and survival across a shell update, acknowledged stale fallback, ordinary 4xx non-fallback, transient 5xx fallback, authoritative deletion, offline cached navigation, and explicit 503 for cache misses or clients that cannot establish the freshness UI. `scripts/check_pwa_commit_regressions.py` exercises representation-aware instant-navigation commit correlation and full-navigation warning persistence. `scripts/check_pwa_slow_convergence.py` exercises the 1500 ms soft timeout, cache-miss waiting, `checking` presentation, non-aborted background verification, terminal-state convergence, full-navigation fallback without a pre-existing client acknowledgement, worker restart ordering, previous-document completion, and newer same-URL commit ordering. `scripts/check_pwa_capabilities.py` separately exercises the live `templates:get-freshness-capabilities` message contract, including the document cache, Glossary cache, integrated Glossary model URL, and `softTimeoutMs`. The Pages build separately verifies that the manifest, icon, service worker, registration script, manifest link, theme-color metadata, `/site-version.json`, and per-page revision metadata exist and satisfy their contracts in the generated artifact.
...
`.github/workflows/mobile-visual-regression.yml` is a pull-request-only consumer of that build artifact for same-repository pull requests targeting `site`. It has only `contents: read` and `actions: read`, waits for the matching successful `build-pages.yml` run at the exact pull-request head SHA, downloads that run's `github-pages` artifact, installs the controller pinned by `requirements-visual.txt` plus its matching Playwright Chromium build, then runs `scripts/check_mobile_layout.py`, `scripts/check_pwa_freshness.py`, `scripts/check_pwa_commit_regressions.py`, `scripts/check_pwa_slow_convergence.py`, and `scripts/check_pwa_capabilities.py` against the already-built site. The layout checker measures 360×800, 390×844, and 412×915 viewports, rejects page-wide horizontal overflow and mobile-density regressions, verifies full repository revisions remain on one line inside their local table scroll container, and enforces the 48 px portal-action floor. The PWA checks verify document revalidation, runtime document-cache fallback/deletion, stale indication, representation-aware commit boundaries, static-shell convergence, Service Worker update propagation, the 1500 ms soft-timeout convergence lifecycle and race ordering, and the live capability vocabulary/URLs/cache namespaces. The workflow uploads screenshots, `metrics.json`, `pwa-freshness.json`, `pwa-document-commit.json`, `pwa-slow-convergence.json`, and `pwa-capabilities.json` as short-lived review evidence. It does not build or deploy Pages and does not run repository code from fork pull requests.
...
`requirements.txt` pins Zensical and build-time syntax-highlighting dependencies, including Pygments. `requirements-visual.txt` separately pins the Playwright controller used only by the mobile visual regression workflow; its matching Chromium build is installed by Playwright rather than committed to the repository. Update either dependency set intentionally, run the relevant full build, mobile layout, PWA freshness lifecycle, document-commit regression, slow-network convergence, and capability checks, and review generated navigation, glossary output, translation-reader routes and switchers, complete repository trees, both copyable-template trees, inline previews, the static repository browser, the guided navigation surface, canonical URLs, provenance/freshness identity, link-validation results, mobile geometry, all PWA evidence, and screenshots before merging.
...
The mobile layout command uses the Chromium build matched to the pinned Playwright controller and writes screenshots plus `metrics.json` under `build/mobile-visual`. The PWA freshness lifecycle command uses the same Chromium installation and writes `pwa-freshness.json` while validating HTTP-cache revalidation, runtime document-cache update/persistence, visible or acknowledged stale fallback, ordinary 4xx non-fallback, 5xx fallback, authoritative deletion, static-shell convergence, worker update propagation, and explicit 503 for cache misses/fail-closed clients. The document-commit regression writes `pwa-document-commit.json` and verifies representation-aware warning clearing. The slow-network convergence command writes `pwa-slow-convergence.json` and verifies the 1500 ms soft timeout, `checking` presentation, non-aborted network continuation, cache-miss waiting, background convergence, full-navigation behavior, and worker/document generation ordering. The separate capability command writes `pwa-capabilities.json` and validates the live freshness-state message contract, document/Glossary cache fields, integrated Glossary model URL, and `softTimeoutMs`. Use workflow-call revision overrides only for deliberate compatibility testing. Normal builds use the reviewed full-SHA lock file. Repository-tree links, preview URLs, repository-browser snapshots, guided-navigation graph/viewer output, glossary output and inline annotations, and translation reader routes always use the actual checked-out commits.