...
5. Classify the provider diff, prepare exact target-provider plus prospective Composition and Policy checkouts, then advance the reviewed full-SHA lock and machine-facing agent projections together with `scripts/advance_publication_source.py`. The tool compare-and-swaps against the expected current lock and writes `publication-sources.json` only after both provider release descriptors and the resulting projection pass preflight.
