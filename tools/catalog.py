...
OWNER = "https://github.com/TakashiSasaki/templates/tree/models"
SCHEMAS = {
...
def render(record: dict[str, Any]) -> str:
    ...
    if ja:
        out += [f"記述レコード: `{record['id']}`。記述の所有者: Models。", f"対象の規範的所有者: [{_md(record['normativeAuthority']['name'])}]({record['normativeAuthority']['id']})。", f"対象の参照先: {record['resourceId']}", "登録は発見のためのものであり、採用・推奨・規範的所有権の移譲を意味しない。"]
    else:
        out += [f"Record: `{record['id']}`; record authority: Models.", f"Normative owner of subject: [{_md(record['normativeAuthority']['name'])}]({record['normativeAuthority']['id']}).", f"Resource reference: {record['resourceId']}", "Registration is for discovery, not adoption, endorsement, or transfer of normative ownership."]
    out += ["", f"## {labels[1]}", ""]
