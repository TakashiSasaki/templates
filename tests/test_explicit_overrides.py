...
def test_v1_keeps_legacy_implicit_override_behavior(tmp_path: Path) -> None:
    (tmp_path / ".git").mkdir()
    relative = "policy/override.md"
    _write_policy(tmp_path, relative, _policy(OVERRIDABLE_ID, "Legacy replacement"))
...
""",
    ...
    assert validate.run(tmp_path, ".agent-policy.yml") == []
    assert render.run(tmp_path, ".agent-policy.yml") == []
    assert "Legacy replacement" in (tmp_path / "AGENTS.md").read_text(encoding="utf-8")


def test_v1_keeps_legacy_repository_rule_replacement(tmp_path: Path) -> None:
    (tmp_path / ".git").mkdir()
    first = "policy/first.md"
    second = "policy/second.md"
    _write_policy(tmp_path, first, _policy("project.duplicate", "First local rule", 1000))
...
""",
    ...
    assert render.run(tmp_path, ".agent-policy.yml") == []
    agents = (tmp_path / "AGENTS.md").read_text(encoding="utf-8")
    assert "Second local rule" in agents
    assert "First local rule" not in agents
