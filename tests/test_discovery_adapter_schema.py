...
def test_schemas_are_valid_202012():
    for name in ("progressive-discovery-adapter", "discovery-projection"):
        Draft202012Validator.check_schema(json.loads(
            (ROOT / f"schemas/{name}.schema.json").read_text()))
    Draft202012Validator(SCHEMA).validate(VALID)


@pytest.mark.parametrize("patch", [
    {"schema_version": 1}, {"schema_version": 3}, {"schema_version": "2"},
    {"unknown": 1}, {"entries": []}, {"entries": [VALID["entries"][0]] * 2},
...
def test_semantic_conflict_is_structurally_valid():
    ...
    Draft202012Validator(SCHEMA).validate({**VALID, "exclude": [
        {"path": "README.md", "reason": "Deliberate contradictory example"}]})
