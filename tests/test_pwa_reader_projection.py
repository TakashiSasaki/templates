...
class PwaReaderProjectionTests(unittest.TestCase):
    def test_pwa_uses_composition_document_without_site_semantic_duplication(self) -> None:
        ...
        self.assertIn("Site does not redefine either contract family", normalized)
        self.assertIn("Composition-owned sources", normalized)
        self.assertIn("[Policy PWA usage guide](../policy/pwa/)", text)
        self.assertIn("not the reusable application PWA capability authority", normalized)
