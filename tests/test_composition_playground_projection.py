...
class CompositionPlaygroundProjectionTests(unittest.TestCase):
    ...
    def test_projection_matches_canonical_composer_for_every_v1_case(self) -> None:
        ...
        outcome_by_id = {entry["index"]: entry for entry in projection["outcomes"]}
        ...
        for recipe_id in sorted(recipe_by_id):
            ...
            self.assertEqual(recipe_projection["case_count"], len(recipe_projection["cases"]))
            for mask, case in enumerate(recipe_projection["cases"]):
                includes = playground.explicit_includes_for_mask(recipe_projection["optional_components"], mask)
                key, normalized_mask, normalized_includes = playground.case_key(recipe_id, includes, recipe_projection["optional_components"])
                self.assertEqual(mask, normalized_mask)
                self.assertEqual(includes, normalized_includes)
                self.assertNotIn(key, case_keys)
                case_keys.add(key)

                configuration = playground.canonical_configuration(recipe_id, includes)
                try:
                    canonical_recipe, resolved = resolve_configuration(state, configuration)
                ...
                self.assertEqual(playground._selection_reason_masks(canonical_recipe, includes, resolved, expected_edges), case["selection_reason_masks"])
                expected_contract_ids = sorted(contract_id for component_id in resolved for contract_id in component_by_id[component_id]["contract_ids"])
                ...
                expected_materials = [{"component": material.component, "destination": material.destination, "ownership": material.ownership, "sha256": sha256_bytes(material.data)} for material in materials]
                projected_materials = [{field: material_by_id[material_id][field] for field in ("component", "destination", "ownership", "sha256")} for material_id in outcome["material_ids"]]
                sort_key = lambda item: (item["destination"], item["component"], item["sha256"])
                self.assertEqual(sorted(expected_materials, key=sort_key), sorted(projected_materials, key=sort_key), key)
                ...
                self.assertEqual(dict(sorted(Counter(action["action"] for action in actions).items())), outcome["initial_plan"]["action_counts"], key)
                self.assertEqual(0, outcome["initial_plan"]["conflict_count"], key)
