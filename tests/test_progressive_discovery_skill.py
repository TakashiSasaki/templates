...
def test_apply_refuses_target_changed_after_plan(tmp_path: Path) -> None:
    ...
    catalog.write_text(catalog.read_text(encoding="utf-8").replace("Reference", "Changed"), encoding="utf-8")
    plan = skill._plan(
        target,
        skill._load_adapter(target)[0],
        skill._index_paths(target, skill._load_adapter(target)[0]),
        skill._expected_documents(target, skill._load_adapter(target)[0], skill._discover_inventory_paths(target, skill._load_adapter(target)[0]))[0],
        skill._generated_specs(skill._load_adapter(target)[0]),
