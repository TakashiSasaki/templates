...
from scripts.generate_repository_trees import build_tree, entry_label, parse_ls_tree
from scripts.generate_repository_trees_composition import generate
...
class RepositoryTreeGenerationTests(unittest.TestCase):
    ...
    def test_generation_uses_tracked_entries_and_immutable_links(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            composition_page = (
                output_root / "docs/repository-trees/composition.md"
            ).read_text(encoding="utf-8")
            index_page = (
                output_root / "docs/repository-trees/index.md"
            ).read_text(encoding="utf-8")

            self.assertIn(revisions["composition"], composition_page)
            self.assertIn(
                f"https://github.com/TakashiSasaki/templates/tree/"
                f"{revisions['composition']}",
                composition_page,
            )
            self.assertIn("a%20%26%20b.txt", composition_page)
            self.assertIn("%3Cscript%3E.txt", composition_page)
            self.assertNotIn("<script>.txt", composition_page)
            self.assertIn("&lt;script&gt;.txt", composition_page)
            self.assertIn("(symlink)", composition_page)
            self.assertNotIn("untracked-secret.txt", composition_page)
            self.assertIn(">source</a>", composition_page)
            self.assertIn("[Composition](composition.md)", index_page)
            self.assertIn(revisions["policy"], index_page)
            self.assertNotIn("GENERATED_REPOSITORY_TREE", composition_page)
            self.assertNotIn("GENERATED_REPOSITORY_TREE_INDEX", index_page)

    def test_generation_requires_composition_and_policy(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            site_root = root / "site"
            output_root = root / "build"
...
class RepositoryTreePreparationTests(unittest.TestCase):
    def make_site_source(self, root: Path) -> None:
        ...
        (root / "docs/index.md").write_text("# Site documentation\n", encoding="utf-8")
    ...
    def test_preparation_augments_only_composition_policy_trees(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            site_root = root / "site"
            ...
            self.assertEqual(
                [child["destination"] for child in manifest["navigation"][1]["children"]],
                [
            ...
            self.assertEqual(
                source_catalog,
                (site_root / "docs/publication-catalog.json").read_text(encoding="utf-8"),
            )
            self.assertEqual(
                source_manifest,
...
class RepositoryTreeConfigurationTests(unittest.TestCase):
    def test_repository_tree_templates_are_present(self) -> None:
        ...
        self.assertFalse((TREE_TEMPLATES / "skill.md").exists())
    ...
    def test_policy_keeps_inventory_separate_from_publication_boundary(self) -> None:
        ...
        self.assertIn("## Repository views", raw_policy)
        self.assertIn("bounded build-time views", policy)
        self.assertIn("Symlinks and gitlinks are never followed", policy)
        self.assertIn("exact Composition and Policy provider revisions", policy)
