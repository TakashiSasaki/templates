...
class UnittestShardTests(unittest.TestCase):
    ...
    def test_named_suites_form_an_exact_partition_of_repository_discovery(self) -> None:
        ...
        self.assertEqual(len(browser_ids), 8)
        self.assertIn(
    ...
    def test_schema_workflow_preserves_full_validation_gate_around_core_and_browser_suites(self) -> None:
        ...
        self.assertNotIn("\n  publication:\n", workflow)
        self.assertIn("\n  primary:\n", workflow)
        self.assertIn("name: publication + preflight + core tests (2/2)", workflow)
        self.assertIn("\n  parallel:\n", workflow)
        self.assertIn("name: core tests (1/2)", workflow)
        self.assertIn("\n  real_browser:\n", workflow)
        self.assertIn("name: real-browser acceptance", workflow)
        self.assertIn("\n  validate:\n", workflow)
        self.assertIn("scripts/materialize_publication.py --source-root .", workflow)
        self.assertIn("scripts/validate_publication.py", workflow)
        self.assertIn("scripts/run_composition_preflight.py fast", workflow)
        self.assertIn("--validators-only", workflow)
        self.assertIn("--publication-already-validated", workflow)
        ...
        self.assertNotIn("PUBLICATION_RESULT:", workflow)
