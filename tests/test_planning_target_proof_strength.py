...
class PlanningTargetProofStrengthTests(unittest.TestCase):
        ...
        return {
            "cli-interface.json": {
                ...
                "entrypoints": [
                    {
                        "id": "records",
                        "purpose": "Expose record operations to CLI callers.",
                    }
                ],
            },
            "service-interface.json": {
                ...
                "operations": [
                    {
                        "id": "records-list",
                        "purpose": "List records through the service boundary.",
                    }
                ],
            },
            "web-interface.json": {
                ...
                "endpoints": [
                    {
                        "id": "records-page",
                        "kind": "browser-page",
                        "purpose": "Render the records page in a browser.",
                    }
                ],
            },
            "mcp-interface.json": {
                ...
                "operations": [
                    {
                        ...
                        "kind": "tool",
                        "transportId": "stdio",
                        "purpose": "List records through MCP.",
                    }
                ],
            },
            "mcp-apps.json": {
                ...
                "views": [
                    {
                        "id": "records-view",
                        "purpose": "Render records in an MCP App View.",
                    }
                ],
                "associations": [
                    {
                        ...
                        "viewId": "records-view",
                        "purpose": "Bind the list tool to its View.",
                    }
                ],
            },
        }
    ...
    def validate(
        self, target: Path, evidence: dict
    ) -> tuple[subprocess.CompletedProcess[str], dict]:
        """Run the complete selected validation chain for integration assertions."""
        self.write_json(
            target / "contracts" / "implementation-evidence.json", evidence
        )
        ...
        try:
            ...
            self.fail(
                "consumer validator did not emit JSON: "
    ...
    def validate_owner(
        ...
        """Run one materialized validator for an owner-specific failure mode.

        Full-chain dispatch is covered by the strong-plan integration test and a
        representative negative test below. Owner-specific negative cases do not
        need to re-run composition-state, validation-runtime probes, or unrelated
        validators merely to assert one component's diagnostic.
        """
        self.write_json(
            target / "contracts" / "implementation-evidence.json", evidence
        )
        registry = json.loads(
            (target / ".template-composition" / "validation-registry.json")
        ...
        self.assertEqual(len(matches), 1, f"unexpected registry entry for {check_id}")
        entry = matches[0]
        entrypoint = target / entry["entrypoint"]
        self.assertTrue(entrypoint.is_file(), f"missing validator: {entrypoint}")
        return subprocess.run(
            [sys.executable, str(entrypoint), *entry["arguments"]],
            cwd=target,
            text=True,
            capture_output=True,
            check=False,
    ...
    def test_strong_target_bound_plan_passes_every_selected_validator_and_worklist_keeps_targets(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for check_id in (
                ...
                self.assertEqual(
                    checks[check_id]["status"], "passed", checks[check_id]
                )
            self.assertIn(
                "planned entrypoint authority", checks["cli-interface"]["stdout"]
            )
            ...
            self.assertEqual(
                scaffold.returncode, 0, scaffold.stdout + scaffold.stderr
            )
    ...
    def test_weak_planning_proof_kinds_fail_at_the_owning_validator_before_coding(
        self,
    ) -> None:
        cases = [
            (
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for requirement_id, weak_kind, expected_check in cases:
                with self.subTest(
                    requirement_id=requirement_id, weak_kind=weak_kind
                ):
                    evidence = self.planning_evidence()
                    self.requirement(evidence, requirement_id)[
                        "requiredPositiveProofKinds"
                    ] = [weak_kind]
                    result = self.validate_owner(target, evidence, expected_check)
                    self.assertNotEqual(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for requirement_id, expected_check in cases:
                with self.subTest(requirement_id=requirement_id):
                    ...
                    self.assertIn("undeclared planned", result.stderr)
    ...
    def test_planned_items_cannot_be_omitted_from_requirement_targets(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            evidence["requirements"] = [
                ...
                if req["id"] != "REQ-PLAN-SERVICE"
            ]
            result = self.validate_owner(target, evidence, "service-interface")
            self.assertNotEqual(result.returncode, 0, result.stdout + result.stderr)
            self.assertIn("missing a planning requirement target", result.stderr)

    def test_planning_evidence_rejects_template_capability_contract(self) -> None:
        """Keep one negative full-chain assertion for failure propagation."""
        with tempfile.TemporaryDirectory() as temp_dir:
            target = self.materialize(Path(temp_dir))
            ...
            self.assertIn(
                "authoritative before coding", checks["cli-interface"]["stderr"]
            )
    ...
    def test_planning_relationships_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = self.validate_owner(
                target, self.planning_evidence(), "mcp-interface"
            )
            self.assertNotEqual(result.returncode, 0, result.stdout + result.stderr)
            self.assertIn("unknown transportId", result.stderr)
            ...
            result = self.validate_owner(
                target, self.planning_evidence(), "mcp-apps"
            )
            self.assertNotEqual(result.returncode, 0, result.stdout + result.stderr)
            self.assertIn(
                "unknown or non-tool planned MCP operation", result.stderr
            )

    def test_unknown_webapp_planning_item_and_wrong_apps_extension_id_fail_closed(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            target = self.materialize(Path(temp_dir))
            unknown_webapp = self.planning_evidence()
            self.requirement(unknown_webapp, "REQ-PLAN-WEBAPP-ROUTE")["targets"][
                0
            ]["itemId"] = "not-a-route"
            result = self.validate_owner(
                target, unknown_webapp, "webapp-implementation-coverage"
            )
            self.assertNotEqual(result.returncode, 0, result.stdout + result.stderr)
            self.assertIn(
                "unknown planning Webapp requirement target", result.stderr
            )

            wrong_extension = self.planning_evidence()
            self.requirement(
                wrong_extension, "REQ-PLAN-MCP-APPS-EXTENSION"
            )["targets"][0]["itemId"] = "other-extension"
            result = self.validate_owner(target, wrong_extension, "mcp-apps")
            self.assertNotEqual(result.returncode, 0, result.stdout + result.stderr)
            self.assertIn("stable extension target id", result.stderr)
