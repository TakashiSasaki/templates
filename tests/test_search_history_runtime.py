...
class SearchHistoryRuntimeContractTests(unittest.TestCase):
    ...
    def test_enter_stages_then_requires_confirmed_navigation_before_commit(self) -> None:
        ...
        self.assertIn("expiresAt: pending.expiresAt", source)
        self.assertNotIn("expiresAt: Date.now() + PENDING_ENTER_SESSION_MAX_AGE_MS", source)
    ...
    def test_precached_runtime_and_built_artifact_browser_checks_are_part_of_ci(self) -> None:
        ...
        self.assertIn('search_host = page.locator("body > *").filter(', review_checker)
        self.assertIn("result = search_host.locator('ol a[href]').first", review_checker)
        ...
        self.assertIn("fallback pagehide extended pending expiry", review_checker)
        self.assertIn("fallback_pagehide_expiry_preserved", review_checker)
