...
def test_work_ledger_guidance_specifies_hub_and_orphan_invariants() -> None:
    ...
    assert "hub projection branch (`main`)" in text
...
def test_stacked_pr_workflow_enforces_intra_authority_stacking() -> None:
    ...
    assert "never create a single pr or git commit branch that spans multiple component orphan histories" in text.lower()
    assert "Landing and projection advance" in text
    assert "The hub branch is never mutated to land intermediate, unmerged component candidate commits" in text


def test_mutation_plan_validation_under_hub_and_orphan() -> None:
    # Verify logical invariant checker for mutation units in Hub-and-Orphan topology
