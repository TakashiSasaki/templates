...
APPLICABILITY_RULE = ROOT / "policy" / "pull-request" / "review-result-applicability.md"
...
REACQUISITION_RULE = ROOT / "policy" / "pull-request" / "review-reacquisition-after-disposition.md"
GITHUB_DISCOVERY = ROOT / "skills" / "pr-merge-gate" / "references" / "github-review-result-discovery.md"
...
def test_clean_submission_cannot_hide_finding_on_another_surface() -> None:
    ...
    assert "clean review body does not negate an actionable inline thread or ordinary comment" in adapter
...
def test_latest_request_is_purpose_aware_not_timestamp_only() -> None:
    ...
    assert "does not supersede a different purpose merely because its request is newer" in policy
...
def test_exact_head_completion_and_historical_finding_applicability_are_separate() -> None:
    ...
    assert "it does not erase an earlier finding whose causal condition remains applicable" in reacquisition
    assert "do not discard an older finding solely because completion evidence for its source review became stale" in skill
...
def test_reaction_only_cannot_establish_completed_problem_free_review() -> None:
    ...
    assert "do not interpret an acknowledgement or attention signal as review completion" in policy
...
def test_merge_gate_directly_lists_new_canonical_rules_and_unknown_applicability_block() -> None:
    ...
    assert "`pull-request.bind-review-result-classification-to-applicable-cycle-and-revision`" in skill
