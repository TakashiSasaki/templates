...
def assert_no_portable_collisions(test: unittest.TestCase, destinations: list[str]) -> None:
    ...
    for destination in sorted(destinations, key=lambda value: (value.casefold(), value)):
        ...
        test.assertFalse(
            key == reserved or key.startswith(reserved + "/") or reserved.startswith(key + "/"),
            f"destination conflicts with reserved lock path: {destination}",
        )
...
class ProductionCatalogTests(unittest.TestCase):
            ...
            for registration in self.components[cid].get("contract_registrations", []):
                ...
                for item in registration["version_history"]:
                    ...
                    if "migration" in item:
                        rendered["migration"] = item["migration"]
    ...
    def materialize(self, selected, target: Path, *, write_lock: bool = False) -> None:
        for cid in selected:
            descriptor = self.components[cid]
            root = component_path(cid).parent
            for material in descriptor["materials"]:
                if "source" not in material:
                    continue
                destination = target / material["destination"]
                destination.parent.mkdir(parents=True, exist_ok=True)
                shutil.copy2(root / material["source"], destination)
        if "lifecycle.contract-evolution" in selected:
            path = target / "contracts/manifest.json"
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_text(json.dumps(self.render_manifest(selected), indent=2) + "\n", encoding="utf-8")
        if write_lock:
            lock = target / ".template-composition/lock.json"
            lock.parent.mkdir(parents=True, exist_ok=True)
            lock.write_text(
                json.dumps({"resolved_components": [{"id": cid} for cid in sorted(selected)]}, indent=2) + "\n",
                encoding="utf-8",
            )

    def run_script(self, target: Path, relative: str, *args: str) -> subprocess.CompletedProcess:
        return subprocess.run(
            [sys.executable, str(target / relative), *args],
            cwd=target,
            text=True,
            capture_output=True,
            check=False,
    ...
    def test_catalog_schema_and_closed_inventory(self):
        ...
        self.assertEqual(
            sorted(path.name for path in (ROOT / "components").iterdir() if path.is_dir()),
    ...
    def test_component_descriptors_sources_and_dependencies_are_closed(self):
        ...
        for cid, descriptor in self.components.items():
            with self.subTest(component=cid):
                ...
                self.assertEqual(descriptor["id"], cid)
                self.assertEqual(descriptor["kind"], cid.split(".", 1)[0])
                self.assertNotIn(cid, descriptor["requires"])
    ...
    def test_dependency_graph_is_acyclic(self):
        ...
        def visit(cid):
            if cid in visited:
                return
            self.assertNotIn(cid, visiting, f"dependency cycle at {cid}")
            visiting.add(cid)
            for dep in self.components[cid]["requires"]:
                visit(dep)
            visiting.remove(cid)
            visited.add(cid)
        for cid in self.catalog["components"]:
            visit(cid)
    ...
    def test_contract_registrations_are_globally_unique_and_component_owned(self):
        ...
        for cid, descriptor in self.components.items():
            ...
            for registration in descriptor.get("contract_registrations", []):
                ...
                versions = [x["version"] for x in registration["version_history"]]
                self.assertEqual(versions, list(range(1, registration["document_schema_version"] + 1)))
                for item in registration["version_history"][1:]:
                    self.assertIn(item["migration"], destinations)
        ...
        for cid, descriptor in self.components.items():
            for material in descriptor["materials"]:
                if material["destination"] == "contracts/manifest.json":
                    owners.append((cid, material["ownership"]))
        self.assertEqual(owners, [("lifecycle.contract-evolution", "generated")])
    ...
    def test_recipes_reference_closed_catalog(self):
        ...
        for rid, recipe in self.recipes.items():
            ...
            self.assertEqual(recipe["id"], rid)
            ...
            sets = list(map(set, groups))
            self.assertFalse(sets[0] & sets[1]); self.assertFalse(sets[0] & sets[2]); self.assertFalse(sets[1] & sets[2])
            for cid in set().union(*sets):
                self.assertIn(cid, self.components)
                self.assertNotEqual(self.components[cid]["kind"], "artifact")
    ...
    def test_webapp_recipe_uses_webapp_artifact_and_optional_capabilities(self):
        ...
        for lifecycle in (
            ...
            self.assertIn(lifecycle, closure)
    ...
    def test_recipe_unions_have_portable_single_owners(self):
        for rid, recipe in self.recipes.items():
            ...
            destinations = [
                ...
                for cid in selected
                for material in self.components[cid]["materials"]
            ]
            self.assertEqual(len(destinations), len(set(destinations)), rid)
            assert_no_portable_collisions(self, destinations)

    def test_generated_manifest_is_deterministic(self):
        selected = self.resolve({"artifact.webapp-core", "capability.mcp-apps"})
        self.assertEqual(self.render_manifest(selected), self.render_manifest(reversed(selected)))

    def test_minimal_skill_scaffold_validates_without_capabilities(self):
        with tempfile.TemporaryDirectory() as temp:
            target = Path(temp)
            self.materialize(["artifact.skill-core"], target)
            result = self.run_script(target, ".github/scripts/validate_skill.py", ".")
    ...
    def test_instruction_only_skill_validates(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            skill.write_text(skill.read_text(encoding="utf-8").replace("Selected profiles: template-scaffold", "Selected profiles: instruction-only"), encoding="utf-8")
            result = self.run_script(target, ".github/scripts/validate_skill.py", ".")
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
    ...
    def test_legacy_skill_application_profile_is_rejected(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.materialize(["artifact.skill-core"], target)
            skill = target / "SKILL.md"
            skill.write_text(skill.read_text(encoding="utf-8").replace("Selected profiles: template-scaffold", "Selected profiles: packaged-cli"), encoding="utf-8")
            result = self.run_script(target, ".github/scripts/validate_skill.py", ".")
    ...
    def test_full_skill_projection_with_lifecycle_validates(self):
        recipe = self.recipes["skill"]
        selected = self.resolve({recipe["artifact"], *recipe["optional_components"]})
        with tempfile.TemporaryDirectory() as temp:
            target = Path(temp)
            self.materialize(selected, target, write_lock=True)
            skill = self.run_script(target, ".github/scripts/validate_skill.py", ".")
    ...
    def test_webapp_template_projection_validates(self):
        recipe = self.recipes["webapp"]
        selected = self.resolve({recipe["artifact"]})
        with tempfile.TemporaryDirectory() as temp:
            target = Path(temp)
            ...
            for script in (
                ...
                if script in {"scripts/validate_contracts.py", "scripts/validate_webapp_evidence.py"}:
                    result = self.run_script(target, script)
                else:
                    result = self.run_script(target, script, ".")
    ...
    def test_webapp_manifest_preserves_domain_versions(self):
        ...
        entries = {entry["id"]: entry for entry in self.render_manifest(selected)["contracts"]}
        self.assertEqual(set(entries), {"implementation_evidence", "release_bundle", "release_evidence", "routes", "surfaces", "ui_states", "viewports"})
        self.assertEqual(entries["routes"]["documentSchemaVersion"], 2)
        self.assertEqual(entries["ui_states"]["documentSchemaVersion"], 2)
        self.assertEqual([x["version"] for x in entries["routes"]["versionHistory"]], [1, 2])
        self.assertEqual([x["version"] for x in entries["ui_states"]["versionHistory"]], [1, 2])
