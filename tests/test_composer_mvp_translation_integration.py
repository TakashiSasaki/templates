    ...
    def test_locked_provider_publishes_japanese_composer_mvp(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            composer_record = next(
                ...
                == PurePosixPath("docs/architecture/composer-mvp.md")
            )
            self.assertEqual(composer_record.language, "ja")
            self.assertEqual(
                composer_record.translation_destination,
                PurePosixPath("ja/composition/architecture/composer-mvp.md"),
            )

            composer = (
                docs_root / "ja" / "composition" / "architecture" / "composer-mvp.md"
            )
