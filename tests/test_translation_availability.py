        ...
        pages = [
            {
                "publication": "site",
                "document": "translated",
                "destination": PurePosixPath("site/translated.md"),
            },
            {
                "publication": "site",
                "document": "missing",
                "destination": PurePosixPath("site/missing.md"),
            },
        ]
        return {"site": (root, documents, [])}, pages

    def test_site_content_compiler_rejects_provider_roots(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            base = Path(directory)
            publications, pages = self.prepare(base / "site", stale=True)
            docs_root = base / "output"
            docs_root.mkdir()
            with self.assertRaisesRegex(TranslationPublicationError, "rejects provider checkouts"):
                publish_translations({"policy": publications["site"]}, pages, docs_root)

    def test_site_owned_stale_translation_remains_available(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            base = Path(directory)
            publications, pages = self.prepare(base / "site", stale=True)
            docs_root = base / "output"
            ...
            self.assertTrue((docs_root / "ja" / "site" / "translated.md").exists())

    def test_current_translation_still_publishes_in_availability_mode(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            base = Path(directory)
            publications, pages = self.prepare(base / "site", stale=False)
            docs_root = base / "output"
            docs_root.mkdir()
            records = publish_translations(
                publications,
            ...
            self.assertEqual(len(records), 1)
            self.assertTrue((docs_root / "ja" / "site" / "translated.md").is_file())

    def test_reader_coverage_distinguishes_stale_and_missing(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            publications, pages = self.prepare(Path(directory) / "site", stale=True)
