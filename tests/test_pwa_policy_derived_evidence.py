...
class PwaPolicyDerivedEvidenceTests(unittest.TestCase):
    ...
    def fixture(self, root: Path, behavior: str) -> object:
        ...
        self.write_json(root, "contracts/pwa-offline.json", {"mode": "product", "routePolicies": [{"routeId": "home", "offlineReadBehavior": behavior}]})
        return load_module(f"pwa_evidence_targets_{behavior}", scripts / "pwa_evidence_targets.py")
    ...
    def evidence(self, targets: list[dict[str, str]]) -> dict:
        ...
        for index, target in enumerate(targets, 1):
            ...
            records.append({"id": record_id, "target": target, "positiveEvidence": [{"id": f"positive-{index:02d}", "kind": "end-to-end-test", "commandId": "browser-proof"}], "negativeEvidence": [{"id": f"negative-{index:02d}", "kind": "end-to-end-test", "commandId": "browser-proof"}]})
    ...
    def test_network_only_policy_does_not_require_cached_content_claims(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.write_json(root, "contracts/implementation-evidence.json", self.evidence(targets))
