...
class McpInterfaceContractTests(unittest.TestCase):
            ...
            "transports": [
                {
                    ...
                    "kind": "stdio",
                    "protocolProbe": (
                        "Start the runtime-owned stdio server and perform MCP discovery "
                        "using 2026-07-28 request metadata."
                    ),
                    "success": "Discovery returns the selected server metadata.",
                    "negative": "Unsupported revision metadata is rejected explicitly.",
                }
            ],
            "operations": [
                {
                    ...
                    "name": "records.list",
                    "transportId": "stdio",
                    "success": "Valid arguments return the record collection.",
                    "negative": "Invalid arguments return a bounded protocol/domain error.",
                }
            ],
        }
    ...
    def evidence(
        ...
        *,
        ...
        command = {
            "id": "mcp-proof",
            "command": "python -m unittest tests.test_mcp_contract",
            "purpose": "Exercise MCP discovery, negative protocol behavior, and tool calls.",
        }
        gate = {
            "id": "release",
            "purpose": "Run executable MCP protocol proof.",
            "commandIds": ["mcp-proof"],
        }
        ...
        def record(
            ...
            return {
                ...
                "target": {
                    "kind": "contract-item",
                    "contractId": "mcp_interface",
                    "itemKind": item_kind,
                    "itemId": item_id,
                },
                "implementationBoundary": {
                    "status": "verified",
                    "description": "MCP adapter boundary.",
                    "locator": "app/mcp.py",
                },
                "positiveEvidence": [
                    {
                        ...
                        "status": proof_status,
                        "kind": proof_kind,
                        "description": "Execute the documented successful MCP path.",
                        "locator": "tests/test_mcp_contract.py",
                        "commandId": "mcp-proof",
                        "expectedResult": "documented MCP success behavior",
                    }
                ],
                "negativeEvidence": [
                    {
                        ...
                        "status": proof_status,
                        "kind": proof_kind,
                        "description": "Execute the documented negative MCP path.",
                        "locator": "tests/test_mcp_contract.py",
                        "commandId": "mcp-proof",
                        "expectedResult": "documented MCP rejection/error behavior",
                    }
                ],
        ...
        return {
            ...
            "requirements": [
                {
                    "id": "REQ-MCP-TRANSPORT",
                    "description": (
                        "The selected MCP transport completes a real protocol round trip "
                        "and rejects unsupported protocol metadata."
                    ),
                    "recordIds": [transport_record_id],
                    "requiredPositiveProofKinds": [required_kind],
                },
                {
                    "id": "REQ-MCP-LIST",
                    "description": (
                        "The records.list MCP operation executes through the declared "
                        "transport for callers."
                    ),
                    "recordIds": [operation_record_id],
                    "requiredPositiveProofKinds": [required_kind],
                },
            ],
            "records": [
                record(
                    ...
                    "transport",
                    "stdio",
                    "mcp-transport-positive",
                    "mcp-transport-negative",
                ),
                record(
                    operation_record_id,
                    "operation",
                    "stdio-list-records",
                    "mcp-operation-positive",
                    "mcp-operation-negative",
                ),
            ],
        }
    ...
    def test_descriptor_registers_machine_contract_and_evidence_dependency(self) -> None:
        ...
        self.assertEqual(
            registrations["mcp_interface"]["document"],
            "contracts/mcp-interface.json",
        )
    ...
    def test_transport_and_operation_require_executable_positive_negative_proof(self) -> None:
        ...
        weak_proof = self.run_validator(
            ...
            self.evidence(proof_kind="inspection", requirement_kind="integration-test"),
        )
        ...
        weak_requirement = self.run_validator(
            self.product_contract(),
            self.evidence(proof_kind="integration-test", requirement_kind="inspection"),
        )
    ...
    def test_deferred_executable_proof_is_truthful_but_not_erased(self) -> None:
        result = self.run_validator(
            self.product_contract(), self.evidence(proof_status="deferred")
        )
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)

    def test_product_evidence_cannot_hide_selected_mcp_in_template_mode(self) -> None:
        result = self.run_validator(
            json.loads(SEED.read_text(encoding="utf-8")), self.evidence()
        )
