...
class WebsiteReviewRegressionTests(unittest.TestCase):
    ...
    def fixture(self, root: Path, *, mode: str = "template") -> None:
        ...
        self.write_json(
            ...
            {"mode": mode, "commands": [], "records": [], "requirements": []},
        )
            ...
            records.append(
                {
                    "id": record_id,
                    "target": target,
                    "positiveEvidence": [proof],
                    "negativeEvidence": [dict(proof, id=f"negative-{index:02d}")],
                }
            )
            requirements.append(
                {
    ...
    def test_discovery_metadata_and_canonical_origin_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertNotEqual(result.returncode, 0)
            self.assertIn(
                "robots and sitemap discovery paths must be distinct", result.stderr
            )
        ...
        for field, value in blank_cases:
            with self.subTest(field=field, value=ord(value[-1])), tempfile.TemporaryDirectory() as temp_dir:
                root = Path(temp_dir)
                self.fixture(root)
    ...
    def test_structure_titles_discovery_route_namespace_and_deep_links_fail_closed(self) -> None:
        for blank in ("   ", "\u115f", "\u3164", "\uffa0"):
            with self.subTest(blank=repr(blank)), tempfile.TemporaryDirectory() as temp_dir:
                root = Path(temp_dir)
                self.fixture(root)
                structure = load(root / "contracts/site-structure.json")
                structure["pages"][0]["title"] = blank
                self.write_json(root, "contracts/site-structure.json", structure)
                result = self.run_validator(CONTRACT_VALIDATOR, root)
                self.assertNotEqual(result.returncode, 0)
                self.assertIn("title must contain at least one visible character", result.stderr)

        for discovery_kind, route_field, collision_path in (
            ("robots", "path", "/robots.txt"),
            ("sitemap", "alias", "/sitemap.xml"),
            ("feed", "alias", "/feed.xml"),
        ):
            with self.subTest(discovery_kind=discovery_kind), tempfile.TemporaryDirectory() as temp_dir:
                root = Path(temp_dir)
                self.fixture(root)
                routes = load(root / "contracts/routes.json")
                if route_field == "path":
                    routes["routes"][0]["path"] = collision_path
                else:
                    routes["routes"][0]["aliases"] = [collision_path]
                self.write_json(root, "contracts/routes.json", routes)
                if discovery_kind == "feed":
                    discovery = load(root / "contracts/site-discovery.json")
                    discovery["feeds"] = [
                        {
                            "id": "updates",
                ...
                self.assertNotEqual(result.returncode, 0)
                self.assertIn(
                    "discovery path collides with canonical/alias route path",
                    result.stderr,
                )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertNotEqual(result.returncode, 0)
            self.assertIn(
                "requires shared route 'home' to support deepLink", result.stderr
            )
    ...
    def test_website_evidence_ignores_other_components_but_requires_browser_linkage(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            browser_record = next(
                ...
                if record["target"].get("contractId") == "browser_identity"
            )
            for requirement in evidence["requirements"]:
                requirement["recordIds"] = [
                    record_id
                    for record_id in requirement.get("recordIds", [])
                    if record_id != browser_record["id"]
                ]
            self.write_json(root, "contracts/implementation-evidence.json", evidence)
            result = self.run_validator(EVIDENCE_VALIDATOR, root)
            self.assertNotEqual(result.returncode, 0)
            self.assertIn(
                "must be linked from at least one product requirement", result.stderr
            )
    ...
    def test_planning_website_evidence_ignores_other_component_targets(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            requirements = [
                ...
                for index, target in enumerate(helper.expected_targets(root), 1)
            ]
            requirements.append(
                {
                    "id": "pwa-plan",
