...
After verification, follow the canonical Skill and the separately pinned
`skills/pr-merge-gate/SKILL.md` source. The landing Skill orchestrates that
shared gate without calling this shim recursively. This route applies to a
single PR and a same-authority stack; it does not authorize merge,
auto-merge, publication, deployment, or cross-authority history changes.
