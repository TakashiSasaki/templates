...
1. **Discovery check**:
   ...
   - If the file is absent: No explicit Composition repository topology is selected. Inspect repository-local authority instructions and Git state; absence does not describe local checkout/worktree layout, prove a unified history, or override independent authority routing.
...
3. **Operational procedure under Hub-and-Orphan topology**:
   ...
   - **Fail-closed response**: Any discovery error, schema violation, branch/mount mismatch, or contract inconsistency MUST immediately halt operations. Do not attempt heuristic repair or infer an undeclared repository topology when a corrupt contract is present.
