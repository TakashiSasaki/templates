...
An explicitly authorized diagnostic audit under human-handoff is selected from the current purpose, candidate, scope, coverage, and bindings. It is not ordinary per-member merge-acceptance evidence, does not authorize merge, does not waive later exact-head review requirements, and must not create a review-retry loop. The workflow-specific stacked procedure may impose exact-head CI sequencing before that audit is requested.
