<!-- agent-policy-generated: true; source-skill: pr-merge-gate -->
...
Use it when clearing submitted review findings or review threads under `pull-request.close-review-threads-before-merge`. Before constructing the backlog, apply the canonical cross-surface discovery and applicability rules and, for GitHub, use `github-review-result-discovery.md` to reconstruct the logical result across submitted reviews, ordinary pull-request comments, inline review comments, resolvable threads, and other applicable provider signals. A material finding remains a review item even when the provider exposes it only in a top-level review body, ordinary comment, or another non-resolvable surface. Track the resulting logical backlog with `review-finding-ledger.md`; the ledger is a transport-independent tracking model, not a new semantic authority. For GitHub-specific acquisition and finding-representation guidance, consult `github-review-finding-representation.md`; that reference is non-normative adapter guidance rather than semantic authority.
